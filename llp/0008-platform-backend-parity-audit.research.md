...
| Timers / event loop / `performance` | Runtime timer queue driven by the host event loop; Android hosts should wake/poll from their Looper/Choreographer integration. `performance.now()` should use an NDK/native monotonic clock rather than wall-clock JS fallback. | Engine-host facility. The Hermes host now exposes native steady-clock `__exactPerformanceNow()` plus runtime `__exactPerformanceTimeOrigin()`, and the JS `performance` singleton consumes them before installing globals. | Keep runtime timers; verify timeout/interval/immediate, cancellation, ref/unref, host wake callback, and monotonic `performance.now()` on Android. |
