//#region src/builtins/string-decoder.js
...
function toDecoderBuffer(buf) {
	...
	if (typeof Buffer !== "undefined" && Buffer.from) {
		if (typeof ArrayBuffer !== "undefined" && buf instanceof ArrayBuffer) return Buffer.from(new Uint8Array(buf));
		if (typeof ArrayBuffer !== "undefined" && ArrayBuffer.isView && ArrayBuffer.isView(buf)) return Buffer.from(buf.buffer, buf.byteOffset || 0, buf.byteLength || buf.length || 0);
...
function bufferToEncoding(buf, encoding, start, end) {
	...
	if (encoding === "base64url") return out.replace(/\+/g, "-").replace(/\//g, "_").replace(/=/g, "");
	return out;
}
...
function utf8Text(buf, offset) {
	...
	if (!this.lastNeed) return buf.toString("utf8", offset);
...
function base64Text(buf, offset) {
	...
	if (remainder === 0) return buf.toString("base64", offset);
	this.lastNeed = 3 - remainder;
	this.lastTotal = 3;
	if (remainder === 1) this.lastChar[0] = buf[buf.length - 1];
	else {
		this.lastChar[0] = buf[buf.length - 2];
		this.lastChar[1] = buf[buf.length - 1];
	}
...
function base64UrlText(buf, offset) {
	return base64Text.call(this, buf, offset).replace(/\+/g, "-").replace(/\//g, "_").replace(/=/g, "");
}
function base64UrlEnd(buf) {
	return base64End.call(this, buf).replace(/\+/g, "-").replace(/\//g, "_").replace(/=/g, "");
}
