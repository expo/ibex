...
function _isWorkerProcess() {
  ...
  return !!(
    process.env &&
    (
      process.env.EXACT_CLUSTER_WORKER ||
...
function Worker(id, childProcess) {
  ...
  this.send = function(message, sendHandle, options, callback) {
    if (!this.process || typeof this.process.send !== 'function') {
      if (typeof callback === 'function') callback(new Error('Worker has no process'));
      return false;
    }
  ...
  this.disconnect = function(callback) {
    ...
    if (typeof callback === 'function') {
      this.process.once('disconnect', callback);
    }
...
function _mergeEnvForCluster(extra) {
  ...
  if (typeof process === 'object' && process !== null && process.env) {
    ...
    for (var key in pEnv) {
      if (Object.prototype.hasOwnProperty.call(pEnv, key)) {
        env[key] = pEnv[key];
      }
    }
  }
...
cluster.fork = function(env) {
  if (cluster.isWorker) {
    throw new Error('cluster.fork() cannot be called from a worker process');
  }
  var workerEnv = env;
  ...
  // Child processes created through cluster have an explicit async online
  // notification in Node. Our compatibility ChildProcess doesn't emit this
  // natively, so emulate it directly.
...
cluster.disconnect = function(callback) {
  ...
  for (var i = 0; i < workerIds.length; i++) {
    ...
    if (worker && typeof worker.disconnect === 'function') {
...
if (cluster.isWorker) {
  cluster.worker = {
    id: (typeof process !== 'undefined' && process && process.env && (process.env.EXACT_CLUSTER_ID || process.env.NODE_UNIQUE_ID))
      ? String(process.env.EXACT_CLUSTER_ID || process.env.NODE_UNIQUE_ID)
      : '0',
    process: process,
