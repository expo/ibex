//#region src/builtins/fs-promises.js
...
FileHandle.prototype.close = function() {
	...
	try {
		var fd = this.fd;
		if (fd !== null && fd !== void 0) fs.closeSync(fd);
		this._fd = null;
...
FileHandle.prototype.truncate = function(len) {
	...
	if (fd === null || fd === void 0) return Promise.reject(/* @__PURE__ */ new Error("File descriptor is not open"));
	try {
		if (typeof fs.ftruncateSync === "function") fs.ftruncateSync(fd, len);
		else if (typeof fs.truncateSync === "function") fs.truncateSync(this._path, len);
		return Promise.resolve();
	} catch (err) {
		return Promise.reject(handleError(err));
	}
...
var promises = {
	...
	access: base.access || function(path, mode) {
		return Promise.resolve(fs.accessSync(path, mode));
	},
	stat: base.stat || function(path) {
		return Promise.resolve(fs.statSync(path));
	},
	lstat: base.lstat || function(path) {
		return Promise.resolve(fs.lstatSync(path));
	},
	mkdir: base.mkdir || function(path, options) {
		fs.mkdirSync(path, options);
		return Promise.resolve();
	},
	rmdir: base.rmdir || function(path) {
		fs.rmdirSync(path);
		return Promise.resolve();
	},
	unlink: base.unlink || function(path) {
		fs.unlinkSync(path);
		return Promise.resolve();
	},
	rename: base.rename || function(from, to) {
		fs.renameSync(from, to);
		return Promise.resolve();
	},
	copyFile: base.copyFile || function(from, to) {
		fs.copyFileSync(from, to);
		return Promise.resolve();
	},
	symlink: base.symlink || function(target, path) {
		fs.symlinkSync(target, path);
		return Promise.resolve();
	},
	link: base.link || function(source, target) {
		fs.linkSync(source, target);
		return Promise.resolve();
	},
	readlink: base.readlink || function(path) {
		return Promise.resolve(fs.readlinkSync(path));
	},
	realpath: base.realpath || function(path) {
		return Promise.resolve(fs.realpathSync(path));
	},
	readdir: base.readdir || function(path) {
		return Promise.resolve(fs.readdirSync(path));
	},
	appendFile: base.appendFile || function(path, data) {
		fs.appendFileSync(path, data);
		return Promise.resolve();
	},
	utimes: base.utimes || function(path, atime, mtime) {
	...
	chown: base.chown || function(path, uid, gid) {
	...
	fdatasync: base.fdatasync || function(fd) {
		return Promise.resolve();
	},
	fsync: base.fsync || function(fd) {
		return Promise.resolve();
	},
	...
	rm: base.rm || function(path, options) {
		fs.rmSync(path, options || {});
		return Promise.resolve();
	},
	mkdtemp: base.mkdtemp || function(prefix) {
		return Promise.resolve(fs.mkdtempSync(prefix));
	},
...
for (var key in base) {
	...
	if (!Object.prototype.hasOwnProperty.call(promises, key)) promises[key] = base[key];
}
