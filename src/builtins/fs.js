...
var _exactPrivateBuiltinBridges =
  typeof __exactPrivateBuiltinBridges === 'object' && __exactPrivateBuiltinBridges
    ...
    : null;
// Armed bootstrap passes this trusted builtin a frozen loader-private capture,
// then seals the root spelling. Calls use only this module-local reference; a
// missing capture fails closed below instead of consulting a mutable global
// later.
// @ref LLP 0022#7-capabilities-principals-and-affordance-parity
var _exactFsMutationGuard =
  _exactPrivateBuiltinBridges &&
  typeof _exactPrivateBuiltinBridges.fsMutationGuard === 'function'
    ? _exactPrivateBuiltinBridges.fsMutationGuard
    : typeof g.__exactFsMutationGuard === 'function'
    ? g.__exactFsMutationGuard
    : null;
// Cwd is a security input. The authenticated builtin wrapper supplies the
// loader-private native function after the raw bridge is sealed, so relative
// filesystem operations never consult mutable `process.cwd`.
...
var _exactGetVirtualCwd =
  ...
  typeof _exactPrivateBuiltinBridges.getVirtualCwd === 'function'
    ...
    : typeof g.__exactGetCwd === 'function'
    ? g.__exactGetCwd
    : null;
...
function _guardClosedFsMutation(operation, path, dest) {
  // @ref LLP 0023#41-the-v1-mutation-surface-small-object-bound-and-completely-specified — Composite aliases must deny before their first metadata lookup or partial effect.
  if (typeof _exactFsMutationGuard !== 'function') {
    throw _makeFsError({ code: 'EPERM' }, operation, path, dest);
  }
  try {
...
function _validateStringWriteEncoding(data, encoding) {
  ...
  if (data.length % 2 !== 0 || /[^0-9a-f]/i.test(data)) {
    var err = new TypeError("The argument 'encoding' is invalid for data of length " + data.length + ". Received '" + encoding + "'");
    err.code = 'ERR_INVALID_ARG_VALUE';
    throw err;
  }
}
...
function _asyncFdOp(native, op, fd, x, y) {
  return native(op, fd, x, y).then(undefined, function(err) {
...
function _asyncWritevFromBuffers(native, fd, buffers, position) {
  var pos = _validateVectoredIoArgs(fd, buffers, position);
  return native(fd, buffers, pos).then(undefined, function(err) {
...
function _withTraversalSlot(limiter, work) {
  return new Promise(function(resolve, reject) {
    ...
    if (limiter.active < limiter.limit) start(); else limiter.queue.push(start);
  });
}
...
function _asyncBuildDirEntries(native, path, options, limiter) {
  limiter = limiter || { active: 0, limit: 32, queue: [] };
  ...
  return _withTraversalSlot(limiter, function() {
    ...
    return _boundedAsyncMap(names, 32, function(name) {
      var fullPath = pathJoin(path, name);
      var statPromise = statNative
        ? _withTraversalSlot(limiter, function() { return _asyncStatImpl(statNative, fullPath, 'lstat', {}); })
        : Promise.resolve().then(function() { try { return lstatSync(fullPath); } catch (_) { return null; } });
      return statPromise.then(function(stat) {
        var entry = new Dirent(_normalizeDirEntryName(name, options && options.encoding), path, stat);
        entry.path = path;
        if (options && options.recursive && stat && stat.isDirectory()) {
          return _asyncBuildDirEntries(native, fullPath, options, limiter).then(function(nested) {
...
function _asyncMkdirSimple(native, path, options) {
  ...
  return _asyncFsPathOp(
      native, 'mkdir', [_nativeMkdirPath(p), null, recursive ? 1 : 0,
        mode === undefined ? -1 : mode], 'mkdir', p)
    .then(function(createdPath) {
      return recursive && createdPath ? createdPath : undefined;
    });
}
...
function _coercePartialWriteError(err, bytesWritten) {
  ...
  if (code !== 'UNKNOWN' && message.indexOf('unknown error, write') === -1) {
    return err;
  }
  var writeErr = new Error('EFBIG: file too large, write');
  writeErr.code = 'EFBIG';
...
function _writeAllSync(fd, bytes, position) {
  ...
  while (offset < bytes.length) {
    ...
    if (typeof written !== 'number' || written <= 0) {
      var err = new Error('write returned zero bytes');
      err.code = 'EIO';
      throw err;
    }
    offset += written;
...
function opendir(path, options, cb) {
  ...
  if (native) {
    ...
    return _deferFsPromiseCallback(
      _asyncBuildDirEntries(native, p, opts || {}).then(function(entries) {
...
function _asyncCp(src, dest, options) {
  _validatePath(src, 'src'); _validatePath(dest, 'dest');
  _guardClosedFsMutation('cp', src, dest);
  var source = _pathToString(src), destination = _pathToString(dest);
  var filter = options.filter;
  if (filter !== undefined && typeof filter !== 'function') throw _fsInvalidArgType('filter', 'function', filter);
  function allowed(s, d) { return filter ? Promise.resolve(filter(s, d)).then(function(v) { return v !== false; }) : Promise.resolve(true); }
  function copyOne(s, d) {
    return allowed(s, d).then(function(ok) {
      if (!ok) return;
      return promises.lstat(s).then(function(st) {
        if (st.isSymbolicLink()) {
          if (options.dereference) return promises.realpath(s).then(function(real) { return copyOne(real, d); });
          return promises.readlink(s).then(function(target) { return promises.symlink(target, d); });
        }
        if (st.isDirectory()) {
          if (!options.recursive) {
            var e = new Error("EISDIR: illegal operation on a directory, copyfile '" + s + "' -> '" + d + "'"); e.code = 'EISDIR'; throw e;
          }
          return promises.mkdir(d, { recursive: true }).then(function() {
            return promises.readdir(s).then(function(names) {
              return names.reduce(function(chain, name) {
                return chain.then(function() { return copyOne(pathJoin(s, name), pathJoin(d, name)); });
              }, Promise.resolve());
            });
          });
        }
        return promises.stat(d).then(function() { return true; }, function() { return false; }).then(function(exists) {
          if (exists && options.force === false) {
            if (options.errorOnExist) { var e = new Error('EEXIST: file already exists'); e.code = 'EEXIST'; throw e; }
            return;
          }
          return promises.copyFile(s, d, options.mode).then(function() {
            return promises.chmod(d, st.mode & 0o7777).then(function() {
              return options.preserveTimestamps ? promises.utimes(d, st.atime, st.mtime) : undefined;
            });
          });
        });
      });
...
function _asyncGlob(pattern, options) {
  var cwd = options.cwd || _currentProcessCwd();
  _validatePath(cwd, 'cwd'); _validatePath(pattern, 'pattern'); _validateEncodingOption(options);
    ...
    return promises.readdir(root, { withFileTypes: true }).then(function(entries) {
      return entries.reduce(function(chain, entry) {
        return chain.then(function() {
          ...
          var excluded = options.exclude && (Array.isArray(options.exclude)
            ? options.exclude.indexOf(candidate) !== -1 : options.exclude(value) === true);
          if (excluded) return;
          if (regex.test(candidate)) out.push(value);
          return entry.isDirectory() ? walk(pathJoin(root, entry.name), candidate) : undefined;
        });
      }, Promise.resolve());
    });
  }
  return walk(_pathToString(cwd), '').then(function() {
    ...
    if (options.encoding === 'buffer') return out.map(function(v) { return Buffer.from(v); });
    return out;
  });
}
...
function copyFile(s, d, modeOrCb, cb) {
  ...
  if (native) {
    ...
    var opName = (mode & constants.COPYFILE_EXCL) === constants.COPYFILE_EXCL ? 'copyfile_excl' : 'copyfile';
    return _deferFsPromiseCallback(
        _asyncFsPathOp(native, opName, [sp, dp], 'copyfile', sp, dp), callback);
  }
...
function exists(path, cb) {
  ...
  if (native) {
    try {
      _validatePath(path); var p = _pathToString(path);
      _asyncFsPathOp(native, 'access', [p, null, 0], 'access', p).then(
        function() { _deferFsCallback(function() { cb(true); }); },
        function() { _deferFsCallback(function() { cb(false); }); });
    } catch (_) { _deferFsCallback(function() { cb(false); }); }
    return;
  }
...
function readv(fd, buffers, position, callback) {
  ...
  try {
    ...
    if (asyncNative) {
      _asyncReadvIntoBuffers(asyncNative, fd, buffers, position).then(
          function(bytesRead) {
...
function writev(fd, buffers, position, callback) {
  ...
  try {
    ...
    if (asyncNative) {
      _asyncWritevFromBuffers(asyncNative, fd, buffers, position).then(
    ...
    try {
      ...
      var readDone = function(err, bytesRead, data) {
        ...
        if (shouldContinue && !rs.destroyed && !rs.closed) {
          if (rs._sawShortRead) {
            setTimeout(function() { rs._read(); }, 1);
      ...
      } else if (useSyncReadFastPath) {
        try {
          var syncBytesRead = readSync(rs.fd, buf, 0, chunkSize, rs._position);
          readDone(null, syncBytesRead, buf);
        } catch (readErr) {
          readDone(readErr);
        }
      } else {
...
function watchFile(filename, options, listener) {
  ...
  if (!watcher) {
    ...
    function asyncWatchStat() {
      return statNative
        ? _asyncStatImpl(statNative, resolvedFilename, 'stat', _extractStatOptions(watcher._statOptions))
        : Promise.resolve().then(function() { return statSync(resolvedFilename, watcher._statOptions); });
    }
    asyncWatchStat().then(function(initial) {
      watcher._prev = initial; watcher._prevExists = true; watcher._hadInitialStat = true;
    }, function() {
      watcher._prevExists = false;
    ...
    watcher._poll = function() {
      ...
      asyncWatchStat().then(function(curr) {
        finish(curr, true);
      }, function() { finish(makeZeroStats(watcher._statOptions.bigint), false); });
      function finish(curr, currExists) {
        ...
        if (watcher._prevExists !== currExists ||
            ...
            (curr.size || 0) !== (watcher._prev.size || 0)) watcher.emit('change', curr, watcher._prev);
        watcher._prev = curr; watcher._prevExists = currExists;
      }
    };
...
function _asyncRm(path, options) {
  _validatePath(path, 'path');
  _guardClosedFsMutation('rm', path);
  if (typeof options === 'boolean') options = { recursive: true, force: true };
  options = options || {};
  ...
  function removeOne(target) {
    return promises.lstat(target).then(function(st) {
      ...
      }).then(function() { return promises.rmdir(target); });
    }, function(err) {
  ...
  function attempt(n) {
    return removeOne(p).catch(function(err) {
      if (force && err && (err.code === 'ENOENT' || err.code === 'ENOTDIR')) return;
      var retryable = err && (err.code === 'EBUSY' || err.code === 'EMFILE' || err.code === 'ENFILE' || err.code === 'ENOTEMPTY' || err.code === 'EPERM');
      if (!retryable || n >= maxRetries) throw _normalizeRmError(err, p, recursive);
      return new Promise(function(resolve) { setTimeout(resolve, retryDelay * (n + 1)); }).then(function() { return attempt(n + 1); });
    });
  }
  return attempt(0);
}
...
function _makeAsyncIteratorFromPromise(valuesPromise) {
  ...
  return {
    next: function() { return iteratorPromise.then(function(it) { return it.next(); }); },
    return: function() { return iteratorPromise.then(function(it) { return it.return(); }); },
    [Symbol.asyncIterator]: function() { return this; }
  };
}
...
var promises = {
  ...
  copyFile: function(s, d, m) {
    return _resolveAsync(function() {
      ...
      if (native) {
        var sp = _pathToString(s); var dp = _pathToString(d);
        var opName = (m & constants.COPYFILE_EXCL) === constants.COPYFILE_EXCL ? 'copyfile_excl' : 'copyfile';
        return _asyncFsPathOp(native, opName, [sp, dp], 'copyfile', sp, dp);
      }
      copyFileSync(s, d, m);
    })();
  },
  ...
  rm: function(p, o) { return _resolveAsync(function() { return _asyncRm(p, o || {}); })(); },
  cp: function(s, d, o) { return _resolveAsync(function() { return _asyncCp(s, d, o || {}); })(); },
  ...
  fdatasync: function(fd) { return _resolveAsync(function() {
    _validateFd(fd);
    var native = _fsAsyncNative('__exactFsFdAsync');
    return native ? _asyncFdOp(native, 'fdatasync', fd) : fdatasyncSync(fd);
  })(); },
  fsync: function(fd) { return _resolveAsync(function() {
    _validateFd(fd);
    var native = _fsAsyncNative('__exactFsFdAsync');
  ...
  lutimes: function(p, a, m) { return _resolveAsync(function() {
    _validatePath(p); _guardClosedFsMutation('lutimes', p); var native = _fsAsyncNative('__exactFsPathAsync'); var pathString = _pathToString(p);
    return native ? _asyncFsPathOp(native, 'lutime', [pathString, null, _toUnixTimestamp(a), _toUnixTimestamp(m)], 'lutimes', pathString) : lutimesSync(p, a, m);
  })(); },
  lchmod: function(p, m) { return _resolveAsync(function() {
    _validatePath(p); m = _coerceMode(m); _validateUint32('mode', m); _guardClosedFsMutation('lchmod', p); var native = _fsAsyncNative('__exactFsPathAsync'); var pathString = _pathToString(p);
    return native ? _asyncFsPathOp(native, 'lchmod', [pathString, null, m], 'lchmod', pathString) : lchmodSync(p, m);
  })(); },
  opendir: function(p, o) { return _resolveAsync(function() {
    _validatePath(p);
    var native = _fsAsyncNative('__exactFsPathAsync');
    var pathString = _pathToString(p);
    return native
  ...
  fchmod: function(fd, m) { return _resolveAsync(function() {
    _validateFdNonNegative(fd); m = _coerceMode(m); _validateUint32('mode', m);
    _guardClosedFsMutation('fchmod');
    var native = _fsAsyncNative('__exactFsFdAsync');
    return native ? _asyncFdOp(native, 'fchmod', fd, m) : fchmodSync(fd, m);
  })(); },
  fchown: function(fd, u, g) { return _resolveAsync(function() {
    _validateFdNonNegative(fd); _validateUidOrGid('uid', u); _validateUidOrGid('gid', g);
    _guardClosedFsMutation('fchown');
    if (u === -1 && g === -1) return;
    var native = _fsAsyncNative('__exactFsFdAsync');
    return native ? _asyncFdOp(native, 'fchown', fd, u, g) : fchownSync(fd, u, g);
  })(); },
  ftruncate: function(fd, l) { return _resolveAsync(function() {
    _validateFd(fd); l = _normalizeTruncateLen(l);
    var native = _fsAsyncNative('__exactFsFdAsync');
...
function fchmod(fd, mode, callback) {
  ...
  if (typeof callback === 'function') {
    ...
    if (asyncNative) {
      return _deferFsPromiseCallback(_asyncFdOp(asyncNative, 'fchmod', fd, mode), callback);
    }
...
function fchown(fd, uid, gid, callback) {
  ...
  if (typeof callback === 'function') {
    var asyncNative = _fsAsyncNative('__exactFsFdAsync');
    if (asyncNative) {
      return _deferFsPromiseCallback(_asyncFdOp(asyncNative, 'fchown', fd, uid, gid), callback);
    }
...
function futimes(fd, atime, mtime, callback) {
  ...
  if (native) {
    return _deferFsPromiseCallback(
        _asyncFdOp(native, 'futimes', fd, _toUnixTimestamp(atime), _toUnixTimestamp(mtime)),
        callback);
  }
  try {
...
function lchmod(path, mode, callback) {
  ...
  var native = _fsAsyncNative('__exactFsPathAsync');
  var p = _pathToString(path);
  if (native) return _deferFsPromiseCallback(_asyncFsPathOp(native, 'lchmod', [p, null, mode], 'lchmod', p), callback);
  return wrapCallback(function() { lchmodSync(path, mode); }, callback, 'lchmod', p);
}
...
function lutimes(path, atime, mtime, callback) {
  ...
  if (native) return _deferFsPromiseCallback(_asyncFsPathOp(native, 'lutime', [p, null, at, mt], 'lutimes', p), callback);
  wrapCallback(function() { lutimesSync(path, atime, mtime); }, callback, 'lutimes', p);
}
