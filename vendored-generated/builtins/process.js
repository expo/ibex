...
function _resolveCwd(path) {
	if (typeof path === "string" && (path.charAt(0) === "/" || /^[A-Za-z]:[\\/]/.test(path))) return path;
	var cwd = typeof process === "object" && process && typeof process.cwd === "function" ? process.cwd() : "/";
	if (cwd.charAt(cwd.length - 1) !== "/") return cwd + "/" + path;
	return cwd + path;
}
...
function _installReadableStdinFallback(proc) {
	...
	stream.resume = function() {
		...
		if (!stream._ended && !stream._pollTimer) (function pollStdin() {
			...
			stream._pollTimer = 0;
			if (stream._encoding && typeof Buffer !== "undefined" && Buffer.from) data = Buffer.from(data, "binary").toString(stream._encoding);
			stream.readableLength += data.length || 0;
			if (typeof stream.emit === "function") stream.emit("data", data);
			stream.readableLength = 0;
	...
	stream.read = function(size) {
		...
		if (data === "") return null;
		if (stream._encoding && data && typeof Buffer !== "undefined" && Buffer.from) return Buffer.from(data, "binary").toString(stream._encoding);
		return data;
	};
