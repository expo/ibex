...
The bind-group layout accepts a UTF-8 label of at most 57 bytes and one through
...
pinned buffer, sampler, texture, or storage-texture layouts. It rejects
external textures, comparison samplers, dynamic/nonzero-min-size buffers,
non-float/non-2d/multisampled textures, and storage textures other than
write-only `rgba16float` 2d. Logical descriptor validation precedes handle and
aggregate-envelope reservation. All three map the same semantic terminal
...
gaps, excluded variants/options, multiple resource members, and UTF-8 label
length.
...
It records the 26-operation triangle subset plus the audited workload closure
