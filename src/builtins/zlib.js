...
function nativeZlibStreamsAvailable() {
  return typeof __exactZlibCreate === 'function' &&
...
function inflateSyncConsumed(bytes, mode, flags) {
  var raw = _nativeInflateSync(bytes, mode, false, flags === undefined ? 2 : flags);
...
function gunzipSync(data, options) {
  ...
  while (remaining.length > 0) {
    ...
    var consumed;
    try {
      var raw = _nativeInflateSync(remaining, 1, false, lenient ? 3 : (2 /* returnConsumed */));
      if (Array.isArray(raw)) {
...
ZlibTransform.prototype._ensureNativeStream = function() {
  ...
  this._nativeId = __exactZlibCreate(
    this._nativeKind,
    this._nativeMode,
    this._level,
    this._strategy,
