//#region src/builtins/util.js
function inherits(constructor, superConstructor) {
	if (typeof constructor !== "function") {
		var e1 = /* @__PURE__ */ new TypeError("The \"ctor\" argument must be of type function. Received " + (constructor === null ? "null" : typeof constructor));
		e1.code = "ERR_INVALID_ARG_TYPE";
		throw e1;
	}
	if (superConstructor === null || typeof superConstructor !== "function" && typeof superConstructor !== "object") {
		var e2 = /* @__PURE__ */ new TypeError("The \"superCtor\" argument must be of type function. Received " + (superConstructor === null ? "null" : typeof superConstructor));
		e2.code = "ERR_INVALID_ARG_TYPE";
		throw e2;
	}
	if (superConstructor.prototype === void 0) {
		var e3 = /* @__PURE__ */ new TypeError("The \"superCtor.prototype\" property must be of type object. Received undefined");
...
function promisify(fn) {
	if (typeof fn !== "function") throw new TypeError("util.promisify requires a function");
	var customSymbol = promisify.custom;
	if (customSymbol != null) {
		var custom = fn[customSymbol];
		if (custom != null) {
			if (typeof custom !== "function") {
				var customErr = /* @__PURE__ */ new TypeError("The promisify custom value must be a function");
	...
	return function() {
		...
		args.push(function(err) {
			...
			if (arguments.length <= 1) return resolve(void 0);
			if (arguments.length === 2) return resolve(arguments[1]);
			var output = [];
			for (var j = 1; j < arguments.length; j++) output.push(arguments[j]);
...
function log() {
	...
	var ts = d + " " + months[now.getMonth()] + " " + (h < 10 ? "0" + h : "" + h) + ":" + (m < 10 ? "0" + m : "" + m) + ":" + (s < 10 ? "0" + s : "" + s);
	var msg = format.apply(null, arguments);
	if (typeof process !== "undefined" && process.stdout && typeof process.stdout.write === "function") process.stdout.write(ts + " - " + msg + "\n");
}
function inspect(value, options) {
	var opts = options || {};
	var depth = opts.depth !== void 0 ? opts.depth : 2;
	var colors = opts.colors || false;
	var breakLength = opts.breakLength !== void 0 ? opts.breakLength : 72;
	var seen = [];
	...
	function _colorize(str, styleType) {
		...
		switch (styleType) {
			case "number":
				codes = ["\x1B[33m", "\x1B[39m"];
				break;
			case "boolean":
				codes = ["\x1B[33m", "\x1B[39m"];
				break;
			case "string":
				codes = ["\x1B[32m", "\x1B[39m"];
				break;
			case "null":
				codes = ["\x1B[1m", "\x1B[22m"];
				break;
			case "undefined":
				codes = ["\x1B[90m", "\x1B[39m"];
				break;
			case "special":
				codes = ["\x1B[36m", "\x1B[39m"];
				break;
			case "regexp":
				codes = ["\x1B[31m", "\x1B[39m"];
				break;
			case "date":
				codes = ["\x1B[35m", "\x1B[39m"];
				break;
			case "symbol":
				codes = ["\x1B[32m", "\x1B[39m"];
				break;
	...
	function _inspect(val, currentDepth) {
		...
		if (val === void 0) return _colorize("undefined", "undefined");
		var t = typeof val;
		...
		if (t === "function") return _colorize("[Function: " + (val.name || "anonymous") + "]", "special");
		...
		var result;
		try {
			if (currentDepth > depth && depth !== null) if (Array.isArray(val)) result = "[Array]";
			else if (val.constructor && val.constructor.name && val.constructor.name !== "Object") result = val.constructor.name;
			else result = "[Object]";
			else if (Array.isArray(val)) if (val.length === 0) result = "[]";
			else {
				var items = [];
				for (var i = 0; i < val.length; i++) items.push(_inspect(val[i], currentDepth + 1));
				...
				if (breakLength > 0 && currentDepth < 10 && singleLine.length > breakLength) {
					...
					result += baseIndent + "]";
				} else result = singleLine;
			}
			else if (val instanceof Date) result = _colorize(val.toISOString(), "date");
			else if (val instanceof RegExp) result = _colorize(String(val), "regexp");
			else if (val instanceof Error) {
				var stack = typeof val.stack === "string" && val.stack ? val.stack : "";
				if (!stack) stack = (val.constructor && val.constructor.name ? val.constructor.name : "Error") + (val.message ? ": " + val.message : "");
				result = stack;
			} else if (typeof val.constructor === "function" && val.constructor.name === "Buffer") result = "<Buffer " + Array.prototype.slice.call(val, 0, Math.min(val.length, 50)).map(function(b) {
				return (b < 16 ? "0" : "") + b.toString(16);
			}).join(" ") + (val.length > 50 ? " ... " + (val.length - 50) + " more bytes" : "") + ">";
			else {
				var ctorPrefix = "";
				if (val.constructor && val.constructor.name && val.constructor.name !== "Object") ctorPrefix = val.constructor.name + " ";
				var keys = Object.keys(val);
				if (keys.length === 0) result = ctorPrefix + "{}";
				else {
					var parts = [];
					for (var ki = 0; ki < keys.length; ki++) {
						...
						var v;
						try {
							v = _inspect(val[k], currentDepth + 1);
						} catch (e) {
							v = "[Getter/Error]";
						}
						parts.push(k + ": " + v);
			...
			seen.pop();
			if (result !== void 0) cache.set(val, result);
		}
		return result;
	}
...
var util = {
	...
	deprecate: function(fn, message, code) {
		if (code !== void 0 && typeof code !== "string") {
			...
			else if (typeof code === "object") if (code.constructor && code.constructor.name) codeDetail = " Received an instance of " + code.constructor.name;
			...
			var codeErr = /* @__PURE__ */ new TypeError("The \"code\" argument must be of type string." + codeDetail);
	...
	types: {
		...
		isTypedArray: function(value) {
			return value && (value instanceof Int8Array || value instanceof Uint8Array || value instanceof Uint8ClampedArray || value instanceof Int16Array || value instanceof Uint16Array || value instanceof Int32Array || value instanceof Uint32Array || value instanceof Float32Array || value instanceof Float64Array || typeof BigInt64Array !== "undefined" && value instanceof BigInt64Array || typeof BigUint64Array !== "undefined" && value instanceof BigUint64Array);
		},
		isNativeError: function(value) {
			return value instanceof Error;
		},
		isNumberObject: function(value) {
			return typeof value === "object" && value instanceof Number;
		},
		isStringObject: function(value) {
			return typeof value === "object" && value instanceof String;
		},
		isBooleanObject: function(value) {
		...
		isBoxedPrimitive: function(value) {
			return value instanceof Number || value instanceof String || value instanceof Boolean || typeof Symbol !== "undefined" && Object.prototype.toString.call(value) === "[object Symbol]" && typeof value === "object" || typeof BigInt !== "undefined" && Object.prototype.toString.call(value) === "[object BigInt]" && typeof value === "object";
		},
		...
		isFloat32Array: function(value) {
			return value instanceof Float32Array;
		},
		isFloat64Array: function(value) {
		...
		isInt8Array: function(value) {
			return value instanceof Int8Array;
		},
		isInt16Array: function(value) {
			return value instanceof Int16Array;
		},
		isInt32Array: function(value) {
		...
		isUint8ClampedArray: function(value) {
			return value instanceof Uint8ClampedArray;
		},
		isUint16Array: function(value) {
			return value instanceof Uint16Array;
		},
		isUint32Array: function(value) {
	...
	TextEncoder: typeof TextEncoder !== "undefined" ? TextEncoder : void 0,
	TextDecoder: typeof TextDecoder !== "undefined" ? TextDecoder : void 0,
	...
	isDeepStrictEqual: function(a, b) {
		...
		if (Array.isArray(a) && Array.isArray(b)) {
			...
			for (var i = 0; i < a.length; i++) if (!util.isDeepStrictEqual(a[i], b[i])) return false;
			return true;
		}
		if (typeof a === "object") {
...
util.parseArgs = function parseArgs(config) {
	...
	var values = {};
	var positionals = [];
	for (var name in options) if (Object.prototype.hasOwnProperty.call(options, name)) {
		var opt = options[name];
		if (opt.default !== void 0) values[name] = opt.default;
	...
	while (i < args.length) {
		...
		if (arg.indexOf("--") === 0) {
			...
			if (optName) {
				...
				if (optDef.type === "boolean") val = val !== void 0 ? val !== "false" : true;
				else if (val === void 0 && i + 1 < args.length) {
					i++;
					val = args[i];
				}
				if (optDef.multiple) {
...
util.debuglog = function debuglog(set) {
	if (typeof set !== "string" || !set) return function() {};
	var normalized = set.toUpperCase();
	if (!(normalized === "NODE_DEBUG" || debugNamespaces[normalized] || debugNamespaces["*"] || debugNamespaces[normalized.toLowerCase()] === true)) return function() {};
...
util.getSystemErrorName = function getSystemErrorName(err) {
	if (typeof err !== "number") {
		var e = /* @__PURE__ */ new TypeError("The \"err\" argument must be of type number. Received type " + typeof err);
		e.code = "ERR_INVALID_ARG_TYPE";
		throw e;
	}
	return _errnoMap[Math.abs(err)] || "Unknown system error " + err;
};
