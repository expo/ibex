...
**Date:** 2026-08-01
**Revised:** 2026-08-01c (round-3 dual-review revision — Fable NOT READY
was not yet in when this landed; Codex 2 IN-DELTA MATERIAL, all
addressed. **The step-0 grammar as drafted admitted zero of the four
...
| bare-callee-miss | 1 | 13 | unprefixed callee names, **two distinct causes**: `oncreate` (10 cells) is a *local nested callback* declared inside `ClientRequest`/`Agent` bodies (http.js:4015, 5877) — an analyzer attribution gap, not an extension point; `IncomingMessage.pause`/`_read`/`resume` (1 each) are framework-virtual stream members, i.e. genuinely preserved-dynamic |
...
- **Bare-callee misses (13 cells), split by cause.** The 10 `oncreate`
  cells are a **local/nested-callback attribution gap**: `oncreate` is
  declared inside the enclosing function body (http.js:4015, 5877) and
  the walker does not attribute nested function declarations. That is
  analyzer work in the same family as closure attribution — *not*
  irreducible extension-point residue, and routing it to register
  item 4 would have approved an analyzer deficiency as unavoidable. Only
  the three `IncomingMessage.*` cells belong to the preserved-dynamic
  residue. (`oncreate` is *also* passed through the documented
  `createConnection` dispatch boundary, so its attribution must handle
  the callback being invoked by caller-supplied code — open question 6.)
...
on dns). What can be said now: the largest mechanism by cell count is
qualified-member miss (155 touching) — but the *proven* yield of the
proposed inheritance resolver is only tls's 30 cells; the rest is
hypothesis pending step 2's diagnosis, and neither figure is a lever
size; the four hook
...
4. **Preserved-dynamic slots** (P1-yes or P2-non-none; blocks step 3's exit): accept that
