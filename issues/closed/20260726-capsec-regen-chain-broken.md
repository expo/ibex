...
The CapSec completion branch contains additional reviewed authenticated-file
and native-graph work inside the first two ranges. After merging the
affirmations above, the composed live ranges remain pinned to
`sha256-CFLFhkyRPRa2_Eu2bZJMb3O57Ulo8LeD7fTM5Inc2ZA` and
`sha256-bA65eLY2kZALZnUkHwbNyg-teFvNI0-xYiJ3wpGEBCw`, respectively. The other
two affirmations merge unchanged. The composed regeneration chain is green at
7,658 coverage edges, 15,316 target cells, and 225 host-task ingress sites.
