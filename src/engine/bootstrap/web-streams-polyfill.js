...
(function () {
  ...
  globalObject.__exactWebStreamsPolyfillLoaded = true;

  if (typeof console !== "undefined" && console && console.log && globalObject && globalObject.process && globalObject.process.env && globalObject.process.env.EX_WEB_STREAMS_POLYFILL_SOURCE === "1") {
    console.log("[exact] web streams polyfill compatibility wrapper beginning");
  }

  var readableStreamPrototype = globalObject.ReadableStream.prototype;
  if (!readableStreamPrototype || typeof readableStreamPrototype.values !== "function") {
    return;
  }

  if (readableStreamPrototype.__exactReadableStreamValuesPatched) {
    if (typeof console !== "undefined" && console && console.log && globalObject && globalObject.process && globalObject.process.env && globalObject.process.env.EX_WEB_STREAMS_POLYFILL_SOURCE === "1") {
      console.log("[exact] readable streams polyfill skipped: already patched values");
    }
    return;
  }

  readableStreamPrototype.__exactReadableStreamValuesPatched = true;

  var OriginalReadableStream = globalObject.ReadableStream;
  if (OriginalReadableStream && !OriginalReadableStream.__exactReadableStreamsPatched) {
    var OriginalReadableStreamDefaultController = globalObject.ReadableStreamDefaultController;
    ...
    var MessagePortConstructor = globalObject.MessagePort;
    var isMessagePort = function (value) {
      return !!MessagePortConstructor && value instanceof MessagePortConstructor;
    };

    var PromiseThen = Promise.prototype && Promise.prototype.then;
    var callThen = function (promise, onFulfilled, onRejected) {
    ...
    var createDataCloneError = function () {
      if (isFunction(globalObject.DOMException)) {
        return new globalObject.DOMException("", "DataCloneError");
      }
      var error = new Error("DataCloneError");
      error.name = "DataCloneError";
      return error;
    };

    if (!globalObject.VideoFrame) {
      var exactVideoFrameCtor = function (data, init) {
        if (!(this instanceof exactVideoFrameCtor)) {
          return new exactVideoFrameCtor(data, init);
        }
        init = init || {};
        ...
        this._exactClosed = false;
      };

      exactVideoFrameCtor.prototype.close = function () {
        this._exactClosed = true;
        this.format = null;
      };

      globalObject.VideoFrame = exactVideoFrameCtor;
    }

    var VideoFrameConstructor = globalObject.VideoFrame;
    var isVideoFrame = function (value) {
      return !!VideoFrameConstructor && isObject(value) && value instanceof VideoFrameConstructor;
    };

    var isVideoFrameClosed = function (frame) {
      return !!(frame && (frame._exactClosed || frame.format === null));
    };

    var cloneVideoFrame = function (frame) {
      return new VideoFrameConstructor(frame._exactData, {
        format: frame.format,
    ...
    var hasTransferMessagePort = function (transfer) {
    ...
    var toTransferList = function (value) {
    ...
    var cloneForOwning = function (value, transfer) {
      if (!isObject(value)) {
        return value;
      }

      if (isVideoFrame(value)) {
        return cloneVideoFrame(value);
      }

      if (value && isObject(value) && 'videoFrame' in value && isVideoFrame(value.videoFrame)) {
        var clone = {};
        for (var key in value) {
          if (key === 'videoFrame') {
            clone.videoFrame = cloneVideoFrame(value.videoFrame);
          } else {
            clone[key] = value[key];
          }
        }
        if (!('videoFrame' in clone)) {
          clone.videoFrame = cloneVideoFrame(value.videoFrame);
        }
        return clone;
      }

      if (transfer.length && isFunction(globalObject.structuredClone)) {
    ...
    var createOwningController = function (controller, source, state) {
      if (controller.__exactOwningFacadeFor === source) {
        return controller.__exactOwningFacade;
      }

      var facade = Object.create(
        OriginalReadableStreamDefaultController && OriginalReadableStreamDefaultController.prototype || Object.prototype
      );
      facade.close = function () {
        return controller.close();
      };
      facade.error = function (error) {
        return controller.error(error);
      };

      Object.defineProperty(facade, "desiredSize", {
        get: function () {
          return controller.desiredSize;
        }
      });

      facade.enqueue = function (chunk, options) {
        if (arguments.length < 2 || options === undefined || options === null) {
        ...
        transfer = toTransferList(transfer);

        if (chunk instanceof ArrayBuffer && transfer.length) {
          throw new TypeError("transfer list is not empty");
        }

        if (isVideoFrame(chunk) && isVideoFrameClosed(chunk)) {
        ...
        var cloned = cloneForOwning(chunk, transfer);

        if (state && state.records && isObject(cloned)) {
          state.records.set(cloned, {
            secondBranchDataCloneError: !isVideoFrame(chunk) && hasTransferMessagePort(transfer)
          });
        }

        if (isVideoFrame(chunk)) {
          chunk.format = null;
        } else if (isObject(chunk) && 'videoFrame' in chunk && isVideoFrame(chunk.videoFrame)) {
          chunk.videoFrame.format = null;
        }

        return controller.enqueue(cloned);
      };

      controller.__exactOwningFacadeFor = source;
      controller.__exactOwningFacade = facade;
      return facade;
    };

    var makeOwningMethod = function (source, name, state) {
      return function () {
        ...
        if (!isFunction(callback)) {
          throw new TypeError("The underlying source's method is not callable");
        }

        if (arguments.length) {
          arguments[0] = createOwningController(arguments[0], source, state);
        }

        return callback.apply(source, arguments);
      };
    };

    var ExactReadableStream = function (source, strategy) {
      if (!(this instanceof ExactReadableStream)) {
        return new ExactReadableStream(source, strategy);
      }

      if (!source || source.type !== 'owning') {
      ...
      var wrappedSource = Object.create(source, {
        type: {
          value: 'owning',
          writable: true,
          configurable: true,
      ...
      wrappedSource.start = makeOwningMethod(source, 'start', state);
      wrappedSource.pull = makeOwningMethod(source, 'pull', state);
      wrappedSource.cancel = makeOwningMethod(source, 'cancel', state);

      var stream = new OriginalReadableStream(wrappedSource, strategy);
      stream.__exactOwningStream = true;
      stream.__exactOwningStreamState = state;
      return stream;
    };

    ExactReadableStream.prototype = OriginalReadableStream.prototype;

    var originalTee = OriginalReadableStream.prototype.tee;
    OriginalReadableStream.prototype.tee = function () {
      if (!this || !this.__exactOwningStream) {
        return originalTee.call(this);
      }

      var state = this.__exactOwningStreamState || {};
      var reader = this.getReader();
      var branchControllers = [void 0, void 0];
      var branchDone = [false, false];

      var closeIfNeeded = function () {
        if (branchDone[0] && branchDone[1]) {
          return reader.cancel();
        }
        return Promise.resolve();
      };

      var createBranch = function (index) {
      ...
      var closed = false;
      var pump = function () {
        if (closed || (branchDone[0] && branchDone[1])) {
          return;
        }

        callThen(reader.read(), function (result) {
          if (result.done) {
            if (!branchDone[0]) {
              branchControllers[0].close();
              branchDone[0] = true;
            }
            if (!branchDone[1]) {
              branchControllers[1].close();
              branchDone[1] = true;
            }
            reader.releaseLock();
            closed = true;
          ...
          var record = state.records && state.records.get(chunk);

          if (!branchDone[0]) {
            try {
              branchControllers[0].enqueue(chunk);
            } catch (error) {
              branchControllers[0].error(error);
              branchDone[0] = true;
            }
          }

          if (!branchDone[1]) {
            if (record && record.secondBranchDataCloneError) {
              branchControllers[1].error(createDataCloneError());
              branchDone[1] = true;
            } else {
              try {
                branchControllers[1].enqueue(chunk);
              } catch (error) {
                branchControllers[1].error(error);
                branchDone[1] = true;
              }
            }
          }
    ...
    var originalPropertyNames = Object.getOwnPropertyNames(OriginalReadableStream);
    for (var p = 0; p < originalPropertyNames.length; p++) {
      if (originalPropertyNames[p] === 'prototype') {
        continue;
      }
      Object.defineProperty(ExactReadableStream, originalPropertyNames[p], Object.getOwnPropertyDescriptor(OriginalReadableStream, originalPropertyNames[p]));
    }

    ExactReadableStream.__exactReadableStreamsPatched = true;

    var sanitizeReadableStreamIterator = function (iterator) {
      ...
      if (typeof iteratorNext !== "function" || typeof iteratorReturn !== "function") {
        return iterator;
      }
      ...
      Object.defineProperty(sanitizedPrototype, "next", {
        ...
        writable: true,
        value: function () {
          return iteratorNext.apply(iterator, arguments);
        }
      });
      Object.defineProperty(sanitizedPrototype, "return", {
        ...
        value: function (value) {
          return iteratorReturn.call(iterator, value);
        }
      });

      if (typeof Symbol === "function" && typeof Symbol.asyncIterator !== "undefined") {
    ...
    var originalValues = OriginalReadableStream.prototype.values;
    var originalGetReader = OriginalReadableStream.prototype.getReader;
    var asyncIteratorSymbol = typeof Symbol === "function" ? Symbol.asyncIterator : null;
    var originalAsyncIterator = asyncIteratorSymbol && OriginalReadableStream.prototype[asyncIteratorSymbol];
    ...
    if (typeof originalGetReader === "function" && !originalGetReader.__exactReadableStreamGetReaderPatched) {
      ...
      OriginalReadableStream.prototype.getReader.__exactReadableStreamGetReaderPatched = true;
    }

    if (typeof originalValues === "function" && !originalValues.__exactReadableStreamValuesPatched) {
      OriginalReadableStream.prototype.values = function () {
        return sanitizeReadableStreamIterator(originalValues.apply(this, arguments));
      };
      OriginalReadableStream.prototype.values.__exactReadableStreamValuesPatched = true;
    }

    if (typeof originalAsyncIterator === "function" && asyncIteratorSymbol && !originalAsyncIterator.__exactReadableStreamAsyncIteratorPatched) {
      OriginalReadableStream.prototype[asyncIteratorSymbol] = function () {
        return sanitizeReadableStreamIterator(originalAsyncIterator.apply(this, arguments));
      };
      OriginalReadableStream.prototype[asyncIteratorSymbol].__exactReadableStreamAsyncIteratorPatched = true;
    }

    if (!OriginalReadableStream.prototype.__exactReadableStreamIteratorPatched) {
      OriginalReadableStream.prototype.__exactReadableStreamIteratorPatched = true;
      globalObject.ReadableStream = ExactReadableStream;
      globalObject.ReadableStream.__exactReadableStreamsPatched = true;
      readableStreamPrototype.__exactReadableStreamValuesPatched = true;
    }
  }
})();
