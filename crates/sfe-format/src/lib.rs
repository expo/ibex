//! Canonical `ibex/single-file-executable/1` envelope encoding and admission.
//!
...
pub const ENVELOPE_SCHEMA_V1: &str = "ibex/single-file-executable/1";
pub const ENTRY_DESIGNATION_SCHEMA_V1: &str = "ibex/entry-designation/1";
pub const STUB_CONTRACT_SCHEMA_V1: &str = "ibex/stub-contract/1";
pub const STUB_CONTRACT_DOMAIN_V1: &str = "ibex:stub-contract:1";
...
pub const FOOTER_MAGIC_V1: [u8; 16] = *b"IBEX_SFE_V1\0\0\0\0\0";
...
impl StubContractV1 {
    fn validate(&self) -> Result<()> {
        ...
        if self.schema != STUB_CONTRACT_SCHEMA_V1
            ...
            || schemas.envelope != ENVELOPE_SCHEMA_V1
    ...
    pub fn digest(&self) -> Result<String> {
        ...
        let value =
            serde_json::to_value(self).map_err(|error| Error::Contract(error.to_string()))?;
        capsec_semantics::digest::compute_domain_digest(STUB_CONTRACT_DOMAIN_V1, &value, &[])
            .map_err(|error| Error::Contract(error.to_string()))
    }
}
...
pub fn build_executable_v1(
    ...
    let directory = EnvelopeDirectoryV1 {
        schema: ENVELOPE_SCHEMA_V1.into(),
...
fn validate_directory(directory: &EnvelopeDirectoryV1) -> Result<()> {
    if directory.schema != ENVELOPE_SCHEMA_V1 {
...
fn write_footer(output: &mut Vec<u8>, footer: FooterV1) {
    output.extend_from_slice(&FOOTER_MAGIC_V1);
    output.extend_from_slice(&FORMAT_VERSION_V1.to_le_bytes());
...
mod tests {
    ...
    fn deterministic_round_trip_bulk_preflights_every_section() {
        ...
        let golden: serde_json::Value =
            serde_json::from_str(include_str!("../tests/fixtures/envelope-v1-golden.json"))
                .unwrap();
        ...
        assert_eq!(admitted.directory.sections.len(), 6);
        assert_eq!(
    ...
    fn malformed_footer_range_digest_overlap_alignment_and_pairing_refuse() {
        ...
        let first = &admitted.directory.sections[0];
        let padding_index = admitted.stub_len + first.offset as usize + first.length as usize;
