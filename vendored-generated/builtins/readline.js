//#region src/builtins/readline.js
...
function _createInvalidArgTypeError(name, expected, value) {
	var err = /* @__PURE__ */ new TypeError("The \"" + name + "\" argument must be of type " + expected + ". Received type " + _typeOf(value));
	err.code = "ERR_INVALID_ARG_TYPE";
	return err;
}
function _createOutOfRangeError(name, range, value) {
	var err = /* @__PURE__ */ new RangeError("The value of \"" + name + "\" is out of range. It must be " + range + ". Received " + String(value));
	err.code = "ERR_OUT_OF_RANGE";
	return err;
}
function _createInvalidArgValueError(name, value) {
	var err = /* @__PURE__ */ new TypeError("The value of \"" + name + "\" is invalid. Received " + String(value));
	err.code = "ERR_INVALID_ARG_VALUE";
	return err;
}
function _createUseAfterCloseError() {
	var err = /* @__PURE__ */ new Error("readline was closed");
...
function getStringWidth(str, removeControlChars) {
	...
	var width = 0;
	var i = 0;
	if (removeControlChars !== false) text = stripVTControlCharacters(text);
...
function charLengthLeft(str, i) {
	if (i <= 0) return 0;
	if (i > 1 && str.codePointAt(i - 2) >= 65536 || str.codePointAt(i - 1) >= 65536) return 2;
	return 1;
}
...
function _formatCompletionBlock(entries, columns) {
	...
	var row;
	var col;
	if (values.length <= 1 || !isFinite(columns) || columns <= 0) return values.join("\r\n");
	for (row = 0; row < values.length; row++) maxWidth = Math.max(maxWidth, getStringWidth(values[row]));
	columnWidth = maxWidth + 2;
	columnCount = Math.max(1, Math.floor(columns / columnWidth));
	if (columnCount <= 1) return values.join("\r\n");
	...
	for (row = 0; row < rowCount; row++) {
		...
		for (col = 0; col < columnCount; col++) {
			...
			entry = values[index];
			line += entry;
			if (col + 1 < columnCount && index + 1 < values.length) line += Array(columnWidth - getStringWidth(entry) + 1).join(" ");
		}
		lines.push(line);
	}
	return lines.join("\r\n");
...
function _formatCompletions(entries, columns) {
	...
	if (block.length > 0) parts.push(_formatCompletionBlock(block, columns));
	return parts.join("\r\n");
}
...
function _emitDecodedKey(stream, sequence, key, escaped) {
	...
	if (sequence.length !== 0 && (key.name !== void 0 || escaped)) {
		stream.emit("keypress", escaped ? void 0 : sequence, key);
		return;
	}
	if (charLengthAt(sequence, 0) === sequence.length) stream.emit("keypress", sequence, key);
}
function _decodeKeyAt(str, start) {
	...
	if (escaped && (ch === "O" || ch === "[")) {
		...
		if (ch === "O") {
			...
			if (ch >= "0" && ch <= "9") while (index < str.length && ch >= "0" && ch <= "9") {
				ch = str.charAt(index);
				s += ch;
				index += 1;
				if (ch < "0" || ch > "9") break;
			...
			if (match) if (match[4]) code += match[4];
			else {
...
function emitKeypressEvents(stream, iface) {
	...
	function onData(input, fromWrite) {
		...
		if (state.pending) {
			if (state.pending === kEscape && state.pendingDeadline > 0 && Date.now() >= state.pendingDeadline) {
				triggerEscape();
		...
		while (index < string.length) {
			...
			if (parsed.needMore) {
				var escapeTimeout = iface.escapeCodeTimeout !== void 0 ? iface.escapeCodeTimeout : kEscapeCodeTimeout;
				var effectiveEscapeTimeout = escapeTimeout === kEscapeCodeTimeout ? Math.max(10, escapeTimeout - kDefaultEscapeTimerSlack) : escapeTimeout;
	...
	if (typeof stream.write === "function" && !stream[KEYPRESS_WRITE_PATCHED]) {
		...
		stream.write = function(chunk) {
			if (this._readableState) this[KEYPRESS_PENDING_WRITES] = (this[KEYPRESS_PENDING_WRITES] || 0) + 1;
			if (!this[KEYPRESS_SUPPRESS_WRITE]) {
				if ((typeof this.listenerCount === "function" ? this.listenerCount("keypress") : 0) > 0) onData(chunk, true);
			}
...
function _parseConstructorArgs(args) {
	...
	var terminal = args[3];
	var options;
	if (input && typeof input === "object" && input.input) options = input;
	else options = {
		input,
		output,
		completer,
...
function Interface() {
	...
	this.terminal = options.terminal !== void 0 ? !!options.terminal : !!(this.output && this.output.isTTY);
	this._promptStr = options.prompt !== void 0 ? String(options.prompt) : "> ";
	this.closed = false;
	this.line = "";
	this._lineBuffer = "";
	this._normalDecoder = new StringDecoder("utf8");
	this.cursor = 0;
	this.history = options.history === void 0 ? [] : options.history;
	this.historyIndex = -1;
	this.historySize = options.historySize === void 0 ? kHistorySize : options.historySize;
	...
	if (this.completer !== void 0 && typeof this.completer !== "function") throw _createInvalidArgValueError("completer", this.completer);
	if (!Array.isArray(this.history)) throw _createInvalidArgTypeError("history", "Array", this.history);
	if (typeof this.historySize !== "number") throw _createInvalidArgTypeError("historySize", "number", this.historySize);
	if (!isFinite(this.historySize) || this.historySize < 0) throw _createOutOfRangeError("historySize", ">= 0", this.historySize);
	if (options.tabSize !== void 0) {
		if (typeof options.tabSize !== "number") throw _createInvalidArgTypeError("tabSize", "number", options.tabSize);
		if (!isFinite(options.tabSize) || options.tabSize <= 0 || options.tabSize % 1 !== 0) {
			if (options.tabSize % 1 !== 0) throw _createOutOfRangeError("tabSize", "an integer", options.tabSize);
			throw _createOutOfRangeError("tabSize", ">= 1", options.tabSize);
		}
		this.tabSize = options.tabSize;
	}
	if (options.escapeCodeTimeout !== void 0) {
		if (typeof options.escapeCodeTimeout !== "number" || !isFinite(options.escapeCodeTimeout)) throw _createInvalidArgValueError("input.escapeCodeTimeout", options.escapeCodeTimeout);
		this.escapeCodeTimeout = options.escapeCodeTimeout;
	}
	if (options.signal !== void 0) {
		if (!options.signal || typeof options.signal.addEventListener !== "function" || typeof options.signal.removeEventListener !== "function") throw _createInvalidArgTypeError("signal", "AbortSignal", options.signal);
	}
	var self = this;
	if (this.input && typeof this.input.on === "function") {
		...
		this._onEnd = function() {
			if (self.closed) return;
			if (!self.terminal && self._normalDecoder) self._lineBuffer += self._normalDecoder.end();
	...
	if (options.signal && typeof options.signal.addEventListener === "function") {
		...
		else options.signal.addEventListener("abort", this._onAbortSignal);
	}
}
Interface.prototype = Object.create(EventEmitter.prototype);
Interface.prototype.constructor = Interface;
Interface.prototype._getColumns = function() {
	if (this.output && typeof this.output.columns === "number" && isFinite(this.output.columns) && this.output.columns > 0) return this.output.columns;
	if (this.input && typeof this.input.columns === "number" && isFinite(this.input.columns) && this.input.columns > 0) return this.input.columns;
	return Infinity;
};
Interface.prototype._getDisplayPos = function(str) {
	...
	while (i < text.length) {
		...
		if (width === 2 && isFinite(col) && col > 0 && (offset + 1) % col === 0) offset += 1;
		offset += width;
		i += char.length;
	}
...
Interface.prototype._insertString = function(value) {
	...
	if (this.historyIndex !== -1 || this._historySearch !== null) this._resetHistorySearch();
	this._lastYank = null;
	this.line = this.line.slice(0, this.cursor) + text + this.line.slice(this.cursor);
	this.cursor += text.length;
	if (this.terminal) if (atEnd) this._writeToOutput(text);
...
Interface.prototype._wordLeftIndex = function() {
	...
	while (index > 0) {
		...
		if (!_isWhitespace(this.line.slice(index - leftLen, index))) break;
		index -= leftLen;
	}
	while (index > 0) {
		var charLen = charLengthLeft(this.line, index);
		if (_isWhitespace(this.line.slice(index - charLen, index))) break;
		index -= charLen;
	}
	return index;
};
Interface.prototype._wordRightIndex = function() {
	...
	while (index < this.line.length) {
		...
		if (!_isWhitespace(this.line.slice(index, index + spaceLen))) break;
		index += spaceLen;
	}
	while (index < this.line.length) {
		var charLen = charLengthAt(this.line, index);
		if (_isWhitespace(this.line.slice(index, index + charLen))) break;
		index += charLen;
	}
	while (index < this.line.length) {
		var tailLen = charLengthAt(this.line, index);
		if (!_isWhitespace(this.line.slice(index, index + tailLen))) break;
		index += tailLen;
	}
	return index;
};
...
Interface.prototype._addHistory = function(line) {
	...
	} else if (this.history[0] === line) return;
...
Interface.prototype._resolveCompletion = function(done) {
	...
	try {
		...
		if (result !== void 0) settle(null, result);
	} catch (err) {
		done(err);
	}
};
...
Interface.prototype._historyStep = function(direction) {
	...
	if (this._historySearch === null) this._historySearch = this.line;
	searchLine = this._historySearch;
	if (direction < 0) {
		index = this.historyIndex >= 0 ? this.historyIndex + 1 : 0;
		for (; index < this.history.length; index++) if (!searchLine || this.history[index].indexOf(searchLine) === 0) {
	...
	for (; index >= 0; index--) if (!searchLine || this.history[index].indexOf(searchLine) === 0) {
		this.historyIndex = index;
		this._replaceLine(this.history[index]);
		return;
	}
...
Interface.prototype._tabComplete = function() {
	...
	this._resolveCompletion(function(err, result) {
		...
		completions = displayCompletions.filter(function(entry) {
			return entry !== "";
		});
	...
	if (key && key.ctrl) {
		...
		if (key.name === "c" || key.name === "d") {
			if (key.name === "d" && this.line.length > 0) {
				if (this.cursor < this.line.length) this._deleteRight();
				this._lastTabLine = null;
				return;
			}
...
Interface.prototype._normalWrite = function(chunk) {
	...
	else if (chunk && this._normalDecoder && typeof Buffer !== "undefined" && typeof Buffer.from === "function") str = this._normalDecoder.write(Buffer.from(chunk));
	else str = chunk && typeof chunk.toString === "function" ? chunk.toString("utf8") : String(chunk);
	if (this._pendingCR) {
		this._lineBuffer += "\n";
		this._pendingCR = false;
		if (str.charAt(0) === "\n" && now - this._lastReturnAt <= this.crlfDelay) str = str.slice(1);
	}
...
Interface.prototype.write = function(data, key) {
	...
	if (this.terminal) {
		for (i = 0; i < str.length; i += charLengthAt(str, i)) {
			var parsed = _decodeKeyAt(str.slice(i, i + charLengthAt(str, i)), 0);
			this._ttyWrite(parsed.escaped ? void 0 : parsed.sequence, parsed.key);
		}
		return;
	}
...
Interface.prototype.close = function() {
	...
	if (this._abortSignal && this._onAbortSignal && typeof this._abortSignal.removeEventListener === "function") this._abortSignal.removeEventListener("abort", this._onAbortSignal);
	if (this.terminal && this.input && typeof this.input.setRawMode === "function") this.input.setRawMode(false);
...
PromiseInterface.prototype.question = function(query, options, cb) {
	if (typeof options === "function" || typeof cb === "function") return Interface.prototype.question.call(this, query, options, cb);
	...
	return new Promise(function(resolve, reject) {
		...
		function cleanup() {
			if (signal && typeof signal.removeEventListener === "function") signal.removeEventListener("abort", onAbort);
			if (self._questionReject === rejectQuestion) self._questionReject = null;
		}
...
if (kPromisifyCustom) Interface.prototype.question[kPromisifyCustom] = function(query, options) {
	return PromiseInterface.prototype.question.call(this, query, options);
};
function createInterface() {
	if (arguments.length === 1 && arguments[0] && typeof arguments[0] === "object" && arguments[0].input) return new Interface(arguments[0]);
	return new Interface(arguments[0], arguments[1], arguments[2], arguments[3]);
}
var promises = {
	Interface: PromiseInterface,
	createInterface: function() {
		if (arguments.length === 1 && arguments[0] && typeof arguments[0] === "object" && arguments[0].input) return new PromiseInterface(arguments[0]);
...
function _validateReadlineCallback(callback) {
	if (callback !== void 0 && typeof callback !== "function") {
		var err = /* @__PURE__ */ new TypeError("The \"callback\" argument must be of type function. Received type " + typeof callback);
		err.code = "ERR_INVALID_ARG_TYPE";
...
function _validateReadlineCoordinate(name, value) {
	if (typeof value === "number" && value !== value) {
		var err = /* @__PURE__ */ new TypeError("The value of \"" + name + "\" is out of range. It must be a valid number. Received NaN");
		err.code = "ERR_INVALID_ARG_VALUE";
...
function cursorTo(stream, x, y, callback) {
	...
	if (typeof x !== "number") {
		if (typeof y === "number") {
			var posErr = /* @__PURE__ */ new TypeError("Cannot set cursor row without setting its column");
	...
	if (typeof y !== "undefined" && typeof y !== "number") y = void 0;
	if (stream && typeof stream.write === "function") if (y !== void 0) stream.write("\x1B[" + (y + 1) + ";" + (x + 1) + "H");
