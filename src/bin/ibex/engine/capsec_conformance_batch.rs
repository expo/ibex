...
#[test]
fn native_harness_fields_are_not_published_as_runtime_results() {
    ...
    remove_native_harness_fields(&mut result);
...
fn remove_native_harness_fields(invocation_result: &mut serde_json::Value) {
    // @ref LLP 0021#wp10--prove-targets-and-publish-the-conformance-report —
    // resultString transports owned cleanup and result-verification data
    // inside the harness; it is not part of the exact public runtime-result
    // evidence schema.
...
async fn execute_native_public_recipe(
    ...
    remove_native_harness_fields(&mut invocation_result);
