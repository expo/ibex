...
`GPUQueue.submit` remains injection-incomplete and has no native codec program:
the wrapper handle sequence does not supply the reviewed sealed command-buffer
records, operation/pass provenance, resource references, finish state, or
canonical program digest. This checkpoint does not substitute handle
references for that missing authority and still installs no public WebGPU
surface, native decoder, semantic service, or provider.
