...
| --- | --- | --- | --- |
| **`OBL-COMPARTMENT-BASELINE`** | Deviation (d)'s real fix: a package compartment must resolve bare globals against a **baseline captured at arming** (endowments + frozen intrinsics), *not* forward to the live realm global — closing the session-`var` / sloppy-assignment / adopted-and-assigned disclosure channel (`var apiKey` at the prompt read by every package). The `[[SessionCreatedVars]]` withhold-list patch was only a partial mitigation. **Filed as ENG-24463; discharged.** The compartment baseline landed 2026-07-12 (`installCompartmentRegistry` / `finalize_compartment_baseline`), and the §7.7(d) fixtures for all three spellings landed 2026-07-15 and pass (re-verified 2026-07-22 on main). Discharge does not by itself advertise a production target — the CLI's default production routes still refuse at the LLP 0033/0035 advertisement gate. | **LLP 0013** | §7.7(d) fixtures for all three spellings: `authenticated_session_package_compartment_withholds_all_late_global_spellings`, `locked_baseline_rejects_prototype_and_contains_late_global_mutation` (pass) |
...
| `OBL-ENGINE-SLICES` | The three engine slices §8 depends on. **Two are landed** in the Hermes patch program: throw-time error capture (stage 1.5) as `0009-raw-throw-capture.patch`, and the completion-record discriminator (§6) as `0010-completion-record-discriminator.patch`. **Trap-free introspection (stage 2) remains.** The required survey of the vendored debugger's inspection path is done: `lib/VM/Debugger/Debugger.cpp` (frame `displayName` read) already performs a trap-free read as the pair `JSObject::getNamedDescriptorPredefined` → `JSObject::getNamedSlotValue` — the descriptor walks the prototype chain and names the containing object without invoking anything, and the slot read bypasses accessors entirely. That pair is the primitive stage 2 should build on rather than inventing a new one. Two properties still need confirming before scoping: that a Proxy receiver yields a null descriptor (degrading to not-found rather than entering a trap), and that the `EXECUTION_STATUS` path cannot itself run project code. | this document (engine) | §8 strata AC 16; `patches/hermes/README.md` rows 0009/0010 |
