...
| 2. Every production surface has classification and target cell | Implemented | The generated registry validates 7,529 coverage edges, 7,830 branches, 15,058 target cells, and 13,386 source references with zero drift. |
...
| 7. Every advertised target has a passing generated report | Safe boundary; promotion deferred | The advertisement set is empty, so no target is falsely claimed. Apple and Windows remain candidates with incomplete public-surface catalogs and cannot be promoted; exact-target completion is tracked separately rather than extending this security-critical implementation tranche. |
...
### 2026-07-28 — executed transport-free TLS socket lifecycle calls

- Promoted exactly five `node:tls` rows: `TLSSocket`, `TLSSocket.close`,
  `TLSSocket.destroy`, `TLSSocket.ref`, and `TLSSocket.unref`. Every receiver
  is constructed without an underlying transport, so construction creates no
  native TLS owner token, engine, selector, listener, or pending timer.
  `ref`/`unref` do not delegate, while `close`/`destroy` update the wrapper and
  schedule the terminal close event that must drain before quiescence.
- Admission is repeated independently by the recipe author, JavaScript
  evidence validator, Rust catalog validator, and loaded-engine JavaScript
  harness. Each binds the actual `src/builtins/tls.js` descriptor, both public
  aliases, canonical `node:tls`, exact empty transport/argument setup, object
  result, and normal-return proof. `connect`, `write`, `end`, TLS server
  construction, and all transport-binding routes remain residual.
- The regenerated Apple catalog digest
  `sha256-zPU1aGpqpzURUAHl8P1d5fCsf1J784wzoiyNFyUSlvk` reports 23,584
  required / **3,708 fully executable** / 3,036 internally verified / 16,840
  unresolved. The regenerated Windows digest
  `sha256-UCyjeXEGrug13rZahxPrEdpehzSwM4Va5pF_jN02SP0` reports 23,243
  required / **3,365 fully executable** / 3,022 internally verified / 16,856
  unresolved. The descriptor-only residual manifest falls from 538 to 533
  rows.
- The EPYC exact-Hermes diagnostic receipt binds the prior 61 focused rows
  plus the five TLS socket calls under catalog digest
  `sha256-AhyXw9-4J0MiqZTNMDbHO02hPHMuW3R5JmPlmGy8D64` and loaded engine
  digest `sha256-MdTBzG7byvsZPmeYDdFD1zx9oP33e7f9Iza6sWNR7LU`. All 66
  fixtures pass and each new row reaches quiescence with zero decisions. The
  raw evidence SHA-256 is
  `4c07d50963eaefd9b74f98c0b37fcadba1fc67fc8cb7ab046c703ebb64ff9a11`;
  independent JavaScript validation accepted all 66 executions and produced
  diagnostic execution digest
  `sha256-LqQHyfPrrOb4xunPqcZskPhhdP8LqqybIo5h4-LyW3M` (artifact SHA-256
  `91cb1c2502220a1ada28ef3f167eebfde132b8118e1e5aac7cad87c848a5dd37`).
- The focused author/catalog/validator/output-accounting suite passes
  **196/196** tests with 125,192 assertions on EPYC Bun 1.3.14; the physical
  Rust exact-Hermes batch passes 66/66. The Linux replay remains diagnostic
...
### 2026-07-28 — prove seven deferred zlib callback wrappers

- Promoted exactly `node_zlib.deflate`, `node_zlib.deflateRaw`,
  `node_zlib.gzip`, `node_zlib.gunzip`, `node_zlib.inflate`,
  `node_zlib.inflateRaw`, and `node_zlib.unzip`. Each public root call receives
  one fixed Buffer plus a dedicated callback credential, returns `undefined`,
  and is credited only after one later error-free callback delivers a nonempty
  encoded byte view or the exact decoded bytes `[105, 98, 101, 120]`.
- The author, independent JavaScript evidence validator, Rust catalog
  validator, and final loaded-engine harness separately repeat the exact
  seven-name allowlist, source descriptor, canonical `node:zlib` alias, input,
  callback contract, root dispatch, undefined return, and deferred output
  proof. Brotli, zstd, `info` engines, arbitrary callbacks, multiple delivery,
  retained codec objects, and incremental stream methods remain residual.
- The first isolated run failed closed in Rust because the generic argument
  gate did not yet recognize the new `zlib-callback` credential. It reached no
  source call and was not credited. After adding exact key and result-contract
  validation, every wrapper passed its own 88/88 static-Hermes process and the
  combined batch passed 94/94.
- The regenerated Apple catalog digest
  `sha256-QmuANSnj9csrf-TVFLUobs3AtChwonR7OCHm_0fOd0Y` reports 23,584
  required / **3,736 fully executable** / 3,036 internally verified / 16,812
  unresolved. The regenerated Windows digest
  `sha256-4Md5AQ6Dmsg2QTGwYZwjK5Zv_2LoD-neUYnrq4LbOsM` reports 23,243
  required / **3,393 fully executable** / 3,022 internally verified / 16,828
  unresolved. The descriptor-only output manifest falls from 514 to 507 rows.
- The EPYC exact-Hermes diagnostic receipt binds the prior 87 focused rows
  plus the seven callback wrappers under catalog digest
  `sha256-mzw-QnyYos7_DXrp8-U-RY7zwlgO71Sd6r7IsHRs_6s` and loaded engine
  digest `sha256-MdTBzG7byvsZPmeYDdFD1zx9oP33e7f9Iza6sWNR7LU`. All 94
  fixtures pass; every new result reports
  `zlibCallbackOutputVerified: true`, quiescence, and zero legacy or typed
  decisions. The raw evidence SHA-256 is
  `c9c1e23ca882f46b546d439dd59f64b1e2c29d72fb4ff05e2714b6485f59cb46`;
  independent JavaScript validation accepted all 94 executions and produced
  diagnostic execution digest
  `sha256-RrJASIEJYQVg0x5YDOcO-zNbR0eTml6NSpqQ-6ocC94` (artifact SHA-256
  `b47d44a11e90ba413333296ffe66b34f65800b25eeb6733ae1a60cfde7f51b35`).
  The diagnostic source binding is baseline revision
  `4c23f5618dbc5ca0180d8aa39eabce6c3547d217` / tree digest
  `sha256-qvXTmQdGjFoBBZuxK3qsaCAjrQSPa8kKplCNRRhQ_mE`.
- The focused author/catalog/validator/output-accounting suite passes
  **218/218** tests with 125,111 assertions on EPYC Bun 1.3.14. A final
  exact-current-file replay also passes the 94/94 physical batch and is
  byte-identical to the raw receipt above.
- Important enforcement mechanisms remain about **99.7% complete**, and the
  overall requested task remains about **96% complete**. Criterion 7 remains
  open and both advertisement sets remain empty.
...
### 2026-07-28 — seal armed exact_crypto controls and converge

- Closed the final known mixed-module control gap without terminal-denying the
  supported crypto module. Armed loading recognizes only the JCS-canonical
  authenticated `exact_crypto` builtin SourceId and, before publishing its
  shared cache record, pins `fips`, `secureHeapUsed`, `setEngine`, and `setFips`
  to `ERR_ACCESS_DENIED` / `CryptoControl`. The `crypto`, `exact:crypto`, and
  `node:crypto` aliases share exact identity and sealed descriptors; unarmed
  diagnostic behavior is unchanged.
- The planned negative control ran the final author/executor against the
  pre-fix loaded engine and failed exactly because `descriptorPinned` was
  false: both FIPS accessor operations and the three compatibility methods
  remained usable. A first hardened smoke then caught a noncanonical SourceId
  literal, and a second caught import-time debug-environment observations; the
  final design corrected the identity and keeps authenticated preload outside
  the zero-decision operation rather than weakening the receipt.
- The fixed EPYC authenticated Apple batch passes **845/845** closed receipts
  against loaded engine
  `sha256-MdTBzG7byvsZPmeYDdFD1zx9oP33e7f9Iza6sWNR7LU`. Independent JavaScript
  reconstruction matches all four new records exactly. Raw evidence SHA-256:
  `08a1585232e19e13d7b9b64f4f6a7b758214a685f564e6a96835be56c4f4577c`.
- Apple catalog
  `sha256-GHlqGTU7b020Cp107EbGh4TShEtB_kOoRz5R1pDtYEo`: 23,598 required /
  **3,928 executable** / 3,042 internally verified / 16,628 unresolved.
  Windows catalog
  `sha256-FAud-eHXcShfXTx3wsnucQFaQ96H2khcNzhxUqxbwzU`: 23,257 /
  **3,564 executable** / 3,028 internally verified / 16,665 unresolved.
- Source-derived outputs and all four example policies were regenerated. The
  six-suite classifier/recipe/template/evidence/root/Android regression passes
  **358/358** with 121,512 assertions; generated drift, `ref-check`, `cargo
  check --bin ibex`, and diff hygiene pass. The library-wide run passes 675,
  ignores 3, and exposes 3 persistent module-runner/VFS assertion failures that
  reproduce serially, including from a clean detached pre-tranche `HEAD`; none
  of their source files is modified by this closure. They are recorded in
  `issues/20260728-preexisting-library-regression-failures.md` instead of
  widening this tranche. Advertisements remain empty and no target-promotion
  claim is made.
- The final upstream check found local `main` at `2afa439b` and fetched
  `origin/main` at `1f5e90a3`; both are ancestors of this branch's pre-closure
  `9a7d7169`, so no merge, rebase, conflict resolution, or generated-artifact
  rotation is required for this checkpoint.
- Per the convergence boundary, this resolves the security-critical CapSec
  rev2 implementation. The bounded completion review found no regression
  caused by this work and no immediately exploitable critical flaw. Remaining
  target-promotion, catalog-completeness, evidence-quality, and non-critical
  design-adjudication work remains visible in prioritized filesystem tickets.

## Completion disposition

CapSec rev2's security-critical implementation is complete. Criteria 1–6 and
8 are implemented; criterion 7 remains an exact release-claim gate, with an
empty advertisement set preventing false promotion. The incomplete candidate
catalogs are residual conformance uncertainty, not permission to infer passing
behavior and not a reason to keep this implementation tranche open.

No new security or conformance tranche follows this milestone. Continue only
through the prioritized filesystem tickets, each with its own bounded scope:

1. P2 target advertisement and complete exact-target reports:
   `issues/20260717-sfe-capsec-advertisement.md`.
2. P2 public-surface catalog/evidence completion:
   `issues/20260728-capsec-public-surface-evidence-backlog.md`.
3. P2 Windows evidence-host installer fallback:
   `issues/20260727-windows-hermes-installer-unauthenticated-gh-fallback.md`.
4. P2 pre-existing module-runner/VFS library regression failures:
   `issues/20260728-preexisting-library-regression-failures.md`.
5. P3 schedule-time principal-capture security adjudication:
   `issues/20260727-schedule-time-capture-security-delta.md`.
6. Non-security operator reliability: secure REPL EOF handling:
