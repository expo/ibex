...
describe("LLP 0033 restricted global corpus runner", () => {
  test("locks all seven preregistered corpora to exact native fixtures", () => {
    ...
    expect(restrictedGlobalCorpusPlan
      .filter((row) => !["observer-equivalence", "source-mutants"].includes(row.id))
      .every((row) => row.tests.length >= 2)).toBe(true);
    expect(restrictedGlobalCorpusPlan
      .find((row) => row.id === "observer-equivalence")?.tests).toEqual([]);
    expect(restrictedGlobalCorpusPlan
      .find((row) => row.id === "source-mutants")?.tests).toEqual([]);
