...
describe("LLP 0026 restricted Exact target report", () => {
  test("keeps every edge incomplete when no per-edge evidence exists", () => {
    const input = fixture();
    const report = buildRestrictedTargetReport(input);
    expect(report.status).toBe("incomplete");
    expect(report.summary.total).toBe(7129);
    expect(report.summary.conformant).toBe(0);
    expect(report.summary.incomplete).toBe(7129);
    expect(report.summary.missingObservations).toBe(14208);
  ...
  test("credits only the exact observed edge and evidence kind", () => {
    ...
    expect(report.summary.incomplete).toBe(7128);
    expect(report.summary.passedObservations).toBe(1);
    expect(report.summary.missingObservations).toBe(14207);
  });
