...
`HermescRecipeV1::production()` and `run_fixed_hermesc` now both include
`-Xes6-block-scoping`; the recipe's domain-separated digest therefore rotates
with the executable semantics. The SFE format test pins the exact argument
vector. A real catalog-compiler regression compiles HBC with the checked-in
`hermesc`, executes it on the matching Hermes VM, and proves per-iteration
capture for a closure-capturing `for...of` loop whose `continue` shape is left
raw by the compatibility transform. `scripts/check-sfe-foundation.sh` runs
that engine-truth leg in the required macOS/Linux SFE foundation CI gate.
