...
impl SourceMap {
    ...
    fn parse(json: &str) -> Option<Self> {
        // Minimal JSON parsing for source maps - extract sources and mappings fields
        let sources = extract_string_array(json, "sources")?;
        let mappings_str = extract_string_field(json, "mappings")?;
        let mappings = decode_vlq_mappings(&mappings_str);
...
/// Extract a string field from JSON (minimal parser).
...
/// Extract a string array field from JSON (minimal parser).
...
mod tests {
    ...
    fn test_extract_string_array() {
        let json = r#"{"sources":["foo.ts","bar.ts"],"mappings":"AAAA"}"#;
        let arr = extract_string_array(json, "sources").unwrap();
        assert_eq!(arr, vec!["foo.ts", "bar.ts"]);
    }
