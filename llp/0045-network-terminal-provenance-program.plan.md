...
llp/evidence/0045-mechanism-matrix-a5ad660ebca02d9155169629564e391f65cd7b1d64fdd87ebf309afe9a8d137f.json
...
genuinely are dynamic. But they are dynamic in **five measured ways, not
two**, and no single mechanism dominates. This plan's job is to make each
...
`llp/evidence/0045-mechanism-matrix-a5ad660e….json`, which carries **one
...
asserts its own invariants (bucket sum equals the Lane B count, touching
counts recompute from the rows, no unrecognized ambiguity shape is
silently bucketed) and **fails** rather than emit an unauditable summary.
...
| unresolved-ident | 17 | 106 | bare `unresolved-call:name` — callbacks, promise bindings, `setTimeout`, and the hook aliases |
...
| bare-callee-miss | 1 | 13 | unprefixed callee names: `oncreate` (the `createConnection` callback of the TLS-bypass hazard) and `IncomingMessage.pause`/`_read`/`resume` — i.e. **preserved-dynamic slots (documented contracts and framework polymorphism) showing up as ambiguity** |

("Cells touching" counts every cell whose bucket contains the mechanism;
a cell with several mechanisms is counted in each. Pure cells sum to 338
with the multi-mechanism buckets in the artifact's `bucketCells`.)
...
prototype-method assignments across **49** unique underscore names. (The
...
- **`const` only.** `var`/`let` are rejected outright rather than
  carrying a lifecycle proof: with `var`/`let` a credited call could
  execute before initialization (hoisting, and CommonJS-cycle exposure
  where a partially-initialized module's export is called during its own
  evaluation), and proving post-initialization dominance for every path
  is a larger analysis than the yield justifies. A `const` binding at
  module top level cannot be observed uninitialized by a call that
  lexically resolves to it without hitting the TDZ error.
...
- **No write of any form** anywhere in the module: assignment, compound
  ...
  target, or catch binding. (`const` makes most of these errors; the
  check stays explicit so the grammar is closed rather than relying on
  the declaration kind.)
- **Use rule / escape predicate.** The alias may appear only as the
  callee of a call expression, or as the `typeof` operand in its own
  guard. Any other appearance — passed as an argument, assigned into an
  object or array, returned, closed over by an escaping function,
  property-accessed — makes the value reachable by code the walker does
  not model, and keeps the refusal. (This is a *use-site* rule: an
  immutable binding captured by a closure is not mutation, but a value
  that escapes can be invoked through a path this resolution does not
  cover.)
...
residual disposition (register item 4). The `bare-callee-miss` cells
(`oncreate`, `IncomingMessage.*`) are these slots surfacing as ambiguity
and belong to this residue.
...
- **Bare-callee misses (13 cells).** `oncreate` and
  `IncomingMessage.pause`/`_read`/`resume` are the extension points
  themselves surfacing as ambiguity. They are resolved by whatever
  preserved-dynamic treatment register item 4 chooses, not by a separate
  mechanism — but they must be counted in that residue, not forgotten.
...
- **paired allow-lists, not "additive-only".** Step 2's resolutions
  ...
  every ambiguity **removal** is individually allow-listed and paired by
  cell, source span, and the resolution proof that justifies it; every
  unlisted field, cell, and ambiguity entry must remain byte-equivalent
  against the prior inventory. An unexplained removal fails the gate
  exactly as an unexplained addition does;
- **adversarial negative fixtures** for every binding and write form the
  step-0 grammar rejects (shadowing, destructuring, update expressions,
...
on dns). What can be said now: the largest single lever is qualified-member
resolution (155 cells touching), not de-virtualization; the four hook
...
4. **Class-2 extension points** (blocks step 3's exit): accept that
