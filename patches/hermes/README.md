...
| 0015 | `0015-empty-disabled-hermes-internal.patch` | B | Keeps the disabled runtime's VM-private HermesInternal helpers only through trusted internal-JavaScript initialization, then removes every own string property and prevents extensions so `EnableHermesInternal=false` publishes a genuinely empty shell. Enabled runtimes retain the historical immutable surface. |

All fifteen apply clean from pristine (`scripts/apply-hermes-patches.sh`) to
tree `647b3490bcfc58134b7a9b4f7e31b3d7943b748e` and
