...
JavaScript to be held to a deadline from outside: refused once it has
passed, and, for **evaluated source**, interrupted mid-way by something
JavaScript cannot catch. Hermes has it in stock: a time-limit monitor and
async-break checks in evaluated source, raising an uncatchable error.
Bytecode carries only the checks it was compiled with, so a deadline is not
by itself a bound on precompiled code. The adapter's contract for it is
LLP 0058.000.000 §8. An engine without an equivalent can host a boot path
but not an effect.
