...
**Date:** 2026-07-10
**Revised:** 2026-07-27 (thirteen exact public loader routes now require a fresh armed runtime, real public `require` traversal, one matching receipt from a loader-private source point, quiescence, engine re-attestation, and zero legacy or typed decisions; a 100-route Apple audit rejected 87 static candidates that bypassed, cached, or entered typed authority, while the retained 13 pass physically against the canonical static-Hermes bundle; the C++ inventory also stops treating preprocessor `defined(...)` and `__has_include(...)` predicates as enclosing functions, removing three fictitious obligations and retargeting affected lifecycle/environment evidence to its real owners; Apple accounting is 3,531 fully executable / 3,134 internally verified / 17,179 unresolved and Windows catalog accounting is 3,190 / 3,120 / 17,193, with both advertisement sets still empty)
