...
- **Windows:** `build.rs` compiles `hermes_runtime_crypto_windows.cc` and
  defines `EXACT_NO_OPENSSL` `[observed]` (`build.rs`). That file registers
  Windows CNG/BCrypt hash, HMAC, PBKDF2, portable scrypt, HKDF, padded AES-CBC,
  and EC key-generation primitives plus stdin/signal noops. EC keys are
  exported as standards-shaped PKCS#8 private and SPKI public PEM; unsupported
  asymmetric operations retain the canonical JavaScript surface's explicit
  failure behavior `[observed]` (`src/engine/hermes_runtime_crypto_windows.cc`).
