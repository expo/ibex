(function portableWebGpuTestWrapperFactory(plan) {
  ...
  function consumesQueueIngress(operationId) {
    return operationId === "GPUQueue.submit" || operationId === "GPUCanvasContext.configure" || operationId === "GPUCanvasContext.unconfigure";
  }
  ...
  function beginPublic(realm, operationId, receiverState, targetState) {
    ...
    var carriedOperationIdentity = route.operationInstanceIdentity.indexOf("not-carried-wrapper-only") !== 0, call = {
      ...
      queueIngressOrdinal: consumesQueueIngress(operationId) && device ? device.nextQueueIngress++ : 0,
  ...
  function assignCallDeviceIngress(call, device) {
    ...
    if (consumesQueueIngress(call.route.operationId)) {
      if (call.queueIngressOrdinal)
        throw Error("queue ingress must be assigned exactly once");
  ...
  defineMethod(prototypeFor("GPUCanvasContext"), "unconfigure", function() {
    var context = requireReceiver(this, "GPUCanvasContext"), wasConfigured = Boolean(context.configuration && context.configuredDevice);
    if (!wasConfigured)
      return;
