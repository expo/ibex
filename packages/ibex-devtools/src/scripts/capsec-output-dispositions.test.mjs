...
describe("LLP 0023 output-disposition dataset", () => {
  ...
  test("accounts for all 7,349 covered surfaces and emits 6,466 context-bound output rows", async () => {
    ...
    expect(catalog.counts).toEqual({
      coverageSurfaces: 7_349,
      outputBearingSurfaces: 5_787,
      structuralOnlySurfaces: 1_557,
      unresolvedSurfaces: 5,
      catalogRows: 6_466,
      parameterizedBindings: 1,
      sourceInventoryRows: 6_056,
      structuredRows: 410,
    });
    ...
    expect(
      ...
      "output-bearing": 5_787,
      "structural-only": 1_557,
      unresolved: 5,
    });
    expect(
      catalog.rows.filter(
  ...
  test("accounts source-shaped globals and source-bound callback outputs", async () => {
    ...
    expect(outputCallbacks).toHaveLength(22);
    expect(
      countsBy(callbacks, (surface) => {
        const edge = edgeByObservedKey.get(surface.observedKey);
        return accountById.get(edge.id).status;
      }),
    ).toEqual({ "output-bearing": 22, "structural-only": 22 });
    ...
    expect(missingOutputCatalog.counts).toEqual({
      ...fixture.catalog.counts,
      outputBearingSurfaces: fixture.catalog.counts.outputBearingSurfaces - 22,
      unresolvedSurfaces: fixture.catalog.counts.unresolvedSurfaces + 22,
      catalogRows: fixture.catalog.counts.catalogRows - 76,
      sourceInventoryRows: fixture.catalog.counts.sourceInventoryRows - 76,
    });
  ...
  test("source-asserts native control tokens and identity-bearing freeze outputs", async () => {
    ...
    expect(rowsFor("__ibexLockedDown").map((row) => row.key)).toEqual([
      {
        surfaceId: edgeByName.get("__ibexLockedDown").id,
        output: "[[value]]",
        alias: "__ibexLockedDown",
        mode: "all",
        sourceKind: "native-op",
        returnVariant: "default",
    ...
    for (const [surfaceName, patchPath] of [
      ...
      expect(rows, surfaceName).toHaveLength(3);
      expect(
      ...
      for (const row of rows.filter(
        (candidate) => candidate.key.output === "[[return]]",
      )) {
  ...
  test("catalogs fixed native callback deliveries separately from ignored callback returns", async () => {
    ...
    expect(rowsFor("__exactDispatchStableEvent").map((row) => row.key)).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          output: "callback:dispatch/0",
          alias: "__exactDispatchStableEvent.binding",
          mode: "json-payload",
          returnVariant: "json-value",
        }),
        expect.objectContaining({
          output: "callback:dispatch/1",
          alias: "__exactDispatchStableEvent.payload",
          mode: "json-payload",
          returnVariant: "json-value",
        }),
        expect.objectContaining({
  ...
  test("derives every host ABI account and output row only from its signature contract", async () => {
    ...
    expect(hostEdges).toHaveLength(318);
  ...
  test("migrates the reviewed policy to exact v2 keys with deterministic accounting", async () => {
    ...
    expect(policy.catalogKeyDigest).toBe(
      "sha256-S73-GuaQjNBkpT6wJcxnGEYx_XFnQMEyTePMM9SK5K4",
    );
    expect(policy.catalogKeyDigest).toBe(catalog.catalogKeyDigest);
    expect(policy.overrides).toHaveLength(374);
    expect(
      new Set(
