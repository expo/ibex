...
export function assertPortablePath(value, label = "portable path") {
...
export function buildFixedVerifierExpectations(policy, sourceRevision, subjectName) {
...
async function walkNoFollow(root) {
  ...
  const visit = async (absolute, relative) => {
    ...
    output.push({ absolute, relative, status, kind });
    if (kind === "directory") {
...
async function validateInstalledPayload(artifactRoot, manifest, context) {
  const payloadRoot = path.join(artifactRoot, "payload");
  const actual = (await walkNoFollow(payloadRoot))
  ...
  for (const expected of manifest.entries) {
    ...
    if (expected.kind === "directory") {
      ...
      assert(await fsp.readlink(observed.absolute) === expected.target, `${expected.path}: installed symlink target drift`);
    } else {
...
function buildReceipt({ manifest, archive, bundle, context, sourceRevision }) {
  return {
...
async function createTransportRecord({ artifactRoot, archivePath, bundlePath, verificationBytes, receipt, context }) {
...
async function validateTransportRecord({ artifactRoot, archiveDigest, manifest, context, sourceRevision, verifyAttestation }) {
  ...
  context.validate(AUTHORITY_SCHEMAS.receipt, receipt, "$ installation receipt");
  assertSame(receipt, buildReceipt({ manifest, archive, bundle, context, sourceRevision }), "installation receipt binding");
...
async function validateTransportRecordShape(artifactRoot, archiveDigest) {
  const transportRoot = path.join(artifactRoot, "LOCAL", "transport", archiveDigest);
  const status = await fsp.lstat(transportRoot, { bigint: true });
...
export async function verifyPortableEngineStore(options, dependencies = {}) {
  const repoRoot = await requireCheckoutRoot(options.repoRoot);
  ...
  const context = options.context ?? await loadCheckedContext(repoRoot, sourceRevision, dependencies);
  const artifactId = options.artifactId;
  assertSemanticDigest(artifactId, "portable artifact ID");
  const storeRoot = await requireStoreRoot(repoRoot);
  ...
  await validateInstalledPayload(artifactRoot, manifest, context);
  ...
  for (const name of transportNames) {
    ...
    await validateTransportRecordShape(artifactRoot, name);
  }
  ...
  const transport = await validateTransportRecord({ artifactRoot, archiveDigest: selectedArchiveDigest, manifest, context, sourceRevision, verifyAttestation });
  return { artifactRoot, manifest, context, transport };
}

async function requireCheckoutRoot(input) {
...
async function ensureDirectoryNoFollow(directory, mode) {
  try {
...
async function requireStoreRoot(repoRoot) {
  const target = path.join(repoRoot, "target");
  const store = path.join(target, "hermes-artifacts");
  await ensureDirectoryNoFollow(target, 0o700);
  await ensureDirectoryNoFollow(store, 0o700);
  return store;
}

async function finalizeFreshStore({ candidateRoot, manifest, archivePath, bundlePath, verificationBytes, receipt, context }) {
  ...
  await createTransportRecord({ artifactRoot: candidateRoot, archivePath, bundlePath, verificationBytes, receipt, context });
  const completion = {
    schema: COMPLETION_SCHEMA,
    artifactId: manifest.artifactId,
...
async function publishAdditionalTransport(candidateRoot, existingRoot, archiveDigest) {
...
export async function installPortableEngine(options, dependencies = {}) {
  const repoRoot = await requireCheckoutRoot(options.repoRoot);
  const sourceRevision = options.expectedSourceRevision;
  assertSourceRevision(sourceRevision, "externally selected checkout revision");
  assertCurrentCheckoutRevision(repoRoot, sourceRevision, dependencies);
  ...
  try {
    ...
    await validateInstalledPayload(candidateRoot, extracted.manifest, context);
    await validateManifestAuthorities(candidateRoot, extracted.manifest, context, sourceRevision, { ...dependencies, repoRoot });
    const receipt = buildReceipt({ manifest: extracted.manifest, archive, bundle, context, sourceRevision });
    context.validate(AUTHORITY_SCHEMAS.receipt, receipt, "$ installation receipt");
    await finalizeFreshStore({ candidateRoot, manifest: extracted.manifest, archivePath: pinnedArchive, bundlePath: pinnedBundle, verificationBytes, receipt, context });
    await validateTransportRecord({
    ...
    try {
      ...
      // Darwin reports EACCES rather than EEXIST when rename(2) targets an
      // existing non-empty directory whose published mode is read-only.
      if (error.code !== "EEXIST" && error.code !== "ENOTEMPTY" && error.code !== "EACCES") throw error;
      // Existing content is never trusted by name. Fully verify it before
      // treating this as idempotence or publishing a distinct authenticated
      // transport encoding for the same portable identity.
      const existing = await verifyPortableEngineStore({
        repoRoot,
      ...
      try {
        await fsp.lstat(destinationTransport);
        destinationExists = true;
      } catch (statusError) {
        if (statusError.code !== "ENOENT") throw statusError;
      }
      if (!destinationExists) await publishAdditionalTransport(candidateRoot, finalRoot, archive.digest);
      const selected = await verifyPortableEngineStore({
        repoRoot,
        expectedSourceRevision: sourceRevision,
        artifactId: extracted.manifest.artifactId,
        archiveDigest: archive.digest,
      ...
      return { ...selected, installed: false, diagnosticOnly: true };
    }
    ...
    return { ...verified, installed: true, diagnosticOnly: true };
  } finally {
