...
function normalize(p) {
  ...
  for (var i = 0; i < parts.length; i++) {
    if (parts[i] === '..') { if (result.length > 0 && result[result.length-1] !== '..') result.pop(); else result.push('..'); }
...
function isAbsolute(p) { return p.charAt(0) === '/'; }
function resolve() {
  var resolved = '';
...
function relative(from, to) {
  ...
  var up = [];
  for (var i = common; i < fromParts.length; i++) up.push('..');
  return up.concat(toParts.slice(common)).join('/');
}
var pathExports = { sep: sep, join: join, normalize: normalize, dirname: dirname, basename: basename, extname: extname, isAbsolute: isAbsolute, resolve: resolve, relative: relative, delimiter: ':',
  parse: function(p) {
    var dir = dirname(p); var base = basename(p); var ext = extname(p);
    return { root: isAbsolute(p) ? '/' : '', dir: dir, base: base, ext: ext, name: base.slice(0, base.length - ext.length) };
  },
  format: function(obj) {
    var dir = obj.dir || obj.root || ''; var base = obj.base || ((obj.name || '') + (obj.ext || ''));
    return dir ? (dir === obj.root ? dir + base : dir + sep + base) : base;
  }
};
pathExports.posix = pathExports;
pathExports.win32 = pathExports;
