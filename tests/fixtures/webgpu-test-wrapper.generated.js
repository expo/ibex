(function portableWebGpuTestWrapperFactory(plan) {
  ...
  function buildRequestDeviceProviderDescriptor(copy) {
    ...
    for (var featureIndex = 0;featureIndex < requestedFeatures.length; featureIndex += 1) {
      ...
      if (plan.fakeClientData.adapterFeatures.indexOf(feature) !== -1 && logicalFeatures.indexOf(feature) === -1)
    ...
    for (var rowIndex = 0;rowIndex < rows.length; rowIndex += 1) {
      var row = rows[rowIndex], logicalValue = row.profileBucket.core, requested = requiredLimits[row.name];
      if (requested !== void 0) {
  ...
  defineMethod(prototypeFor("GPUAdapter"), "requestDevice", function(descriptor) {
    ...
    return promiseOperation(call, function() {
      ...
      var logicalProviderDescriptor = descriptorFailure ? null : buildRequestDeviceProviderDescriptor(copy), descriptorFacts = {};
