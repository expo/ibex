...
already loaded `stream` namespace before manifest resolution. Those names
therefore cannot evidence the declared `node_stream_consumers` or
`node_stream_promises` source bodies today. The table now rejects unrelated
names before calling `_getStreamBuiltins()`, so an arbitrary public import no
longer initializes `node_stream` merely as a side effect of missing this
compatibility table `[observed]` (`src/engine/bootstrap/module-loader.js`).
...
- **Hermetic dependency profile.** rustls uses the `ring` provider for every
  ...
  implementation or trust behavior enters the wire path. That decoder is
  gated behind the default-on `tls-client-identity-openssl` Cargo feature:
  compile-only cross gates and hosts without a working build perl can build
  with `default-features = false`, in which case `pfx` / `passphrase` options
  fail with a typed reduced-TLS-profile error instead of being silently
  mis-parsed (ENG-25390). PFX input is capped
