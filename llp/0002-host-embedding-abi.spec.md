...
`GPUAdapter.requestDevice`, `GPUDevice.createCommandEncoder`,
...
of the provider boundary. The two device-object creation requests admit a
...
only their reviewed converted descriptor: `{label: string}` for the command
encoder and `{label: string, code: string}` for the shader module. Both map the
same three semantic terminal classes: synchronous Web IDL rejection produces
no service call, later predicate rejection produces a typed device-error event,
and admitted success produces an empty RESULT_NONE terminal receipt. The
destroy request admits a bounded canonical
...
The device-object creation vectors cover both the command-encoder and shader-
module target/reference joins, converted descriptors, admitted carrier
projections, and empty successful terminal receipts.
