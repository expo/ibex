...
var _settingDefinitions = [
	{
		key: "headerTableSize",
		id: constants.NGHTTP2_SETTINGS_HEADER_TABLE_SIZE,
		type: "uint32"
	},
	{
		key: "enablePush",
		id: constants.NGHTTP2_SETTINGS_ENABLE_PUSH,
		type: "boolean"
	},
	{
		key: "maxConcurrentStreams",
		id: constants.NGHTTP2_SETTINGS_MAX_CONCURRENT_STREAMS,
		type: "uint32"
	},
	{
		key: "initialWindowSize",
		id: constants.NGHTTP2_SETTINGS_INITIAL_WINDOW_SIZE,
		type: "uint32"
	},
	{
		key: "maxFrameSize",
		id: constants.NGHTTP2_SETTINGS_MAX_FRAME_SIZE,
		type: "frameSize"
	},
	{
		key: "maxHeaderListSize",
		id: constants.NGHTTP2_SETTINGS_MAX_HEADER_LIST_SIZE,
		type: "uint32"
	},
	{
		key: "enableConnectProtocol",
		id: constants.NGHTTP2_SETTINGS_ENABLE_CONNECT_PROTOCOL,
		type: "boolean"
	}
];
...
function _bufferFrom(value) {
	...
	if (_isTypedArray(value)) return new Uint8Array(value.buffer, value.byteOffset || 0, value.byteLength || value.length);
	return value;
}
function _alloc(length) {
	if (typeof Buffer !== "undefined" && typeof Buffer.alloc === "function") return Buffer.alloc(length);
...
function _readUint32BE(buf, offset) {
	return (buf[offset] << 24 >>> 0) + (buf[offset + 1] << 16 >>> 0) + (buf[offset + 2] << 8 >>> 0) + (buf[offset + 3] >>> 0) >>> 0;
}
function _validateSettingsObject(settings) {
	if (typeof settings === "undefined") return {};
	if (!settings || typeof settings !== "object" || Array.isArray(settings)) throw _createError("ERR_INVALID_ARG_TYPE", "The \"settings\" argument must be of type object");
	return settings;
}
function _validateSettingValue(name, value, type) {
	if (type === "boolean") {
		if (typeof value !== "boolean") throw _createError("ERR_HTTP2_INVALID_SETTING_VALUE", "Invalid value for setting \"" + name + "\": " + value);
		return value ? 1 : 0;
	}
	if (typeof value !== "number" || value !== value >>> 0) throw _createError("ERR_HTTP2_INVALID_SETTING_VALUE", "Invalid value for setting \"" + name + "\": " + value);
	if (type === "frameSize" && (value < 16384 || value > 16777215)) throw _createError("ERR_HTTP2_INVALID_SETTING_VALUE", "Invalid value for setting \"" + name + "\": " + value);
	return value >>> 0;
}
function _normalizePackedSettings(settings) {
	var normalized = {};
	if (hasOwn.call(settings, "maxHeaderListSize")) normalized.maxHeaderListSize = settings.maxHeaderListSize;
	if (hasOwn.call(settings, "maxHeaderSize")) {
		if (hasOwn.call(settings, "maxHeaderListSize") && typeof process !== "undefined" && typeof process.emitWarning === "function") process.emitWarning("settings.maxHeaderSize overwrite settings.maxHeaderListSize");
	...
	for (var i = 0; i < _settingDefinitions.length; i++) {
		...
		if (hasOwn.call(settings, definition.key)) normalized[definition.key] = settings[definition.key];
	}
	return normalized;
}
function _createUnsupportedError(methodName) {
	return _createError("ERR_HTTP2_UNSUPPORTED", methodName + " is not supported in this runtime without native HTTP/2 support");
}
...
function getUnpackedSettings(buf) {
	if (!_isTypedArray(buf)) throw _createError("ERR_INVALID_ARG_TYPE", "The \"buf\" argument must be an instance of Buffer or TypedArray");
	var bytes = _bufferFrom(buf);
	if ((bytes.length || 0) % 6 !== 0) throw _createError("ERR_HTTP2_INVALID_PACKED_SETTINGS_LENGTH", "Packed settings length must be a multiple of six");
	var settings = {};
	var custom = null;
	for (var i = 0; i < bytes.length; i += 6) {
		var id = _readUint16BE(bytes, i);
		var value = _readUint32BE(bytes, i + 2);
		if (id === constants.NGHTTP2_SETTINGS_HEADER_TABLE_SIZE) settings.headerTableSize = value;
		else if (id === constants.NGHTTP2_SETTINGS_ENABLE_PUSH) settings.enablePush = value !== 0;
		else if (id === constants.NGHTTP2_SETTINGS_MAX_CONCURRENT_STREAMS) settings.maxConcurrentStreams = value;
		else if (id === constants.NGHTTP2_SETTINGS_INITIAL_WINDOW_SIZE) settings.initialWindowSize = value;
		else if (id === constants.NGHTTP2_SETTINGS_MAX_FRAME_SIZE) settings.maxFrameSize = value;
		else if (id === constants.NGHTTP2_SETTINGS_MAX_HEADER_LIST_SIZE) settings.maxHeaderListSize = value;
		else if (id === constants.NGHTTP2_SETTINGS_ENABLE_CONNECT_PROTOCOL) settings.enableConnectProtocol = value !== 0;
		else {
			if (!custom) custom = {};
			custom[id] = value;
		}
...
Http2ServerRequest.prototype.setTimeout = function(timeout, callback) {
	if (this.stream && typeof this.stream.setTimeout === "function") this.stream.setTimeout(timeout, callback);
	else if (typeof callback === "function" && typeof this.once === "function") this.once("timeout", callback);
	return this;
};
...
Http2ServerResponse.prototype.writeHead = function(statusCode, statusMessage, headers) {
	...
	if (headers) {
		for (var key in headers) if (hasOwn.call(headers, key)) this.setHeader(key, headers[key]);
	}
	return this;
};
Http2ServerResponse.prototype.flushHeaders = function() {
	...
	if (this.stream && typeof this.stream.respond === "function") {
		...
		for (var key in this._headers) if (hasOwn.call(this._headers, key)) headers[key] = this._headers[key];
...
Http2ServerResponse.prototype.write = function(chunk, encoding, callback) {
	...
	if (this.finished) {
		if (typeof callback === "function") callback(/* @__PURE__ */ new Error("write after end"));
		return false;
	}
	if (!this.headersSent) this.flushHeaders();
	if (chunk !== null && typeof chunk !== "undefined") this._body.push(chunk);
	if (this.stream && typeof this.stream.write === "function") return this.stream.write(chunk, encoding, callback);
...
Http2ServerResponse.prototype.end = function(chunk, encoding, callback) {
	...
	if (chunk !== null && typeof chunk !== "undefined") this.write(chunk, encoding);
	else if (!this.headersSent) this.flushHeaders();
	this.finished = true;
	this.writableEnded = true;
	this.writableFinished = true;
	if (this.stream && typeof this.stream.end === "function") this.stream.end();
	if (typeof callback === "function") setTimeout(callback, 0);
