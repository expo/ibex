//#region src/builtins/async-hooks.js
var _hooksEnabled = false;
var _noop = function() {};
var _idCounter = 1;
var _activeHooks = [];
var _resourceFinalizer = typeof FinalizationRegistry === "function" ? new FinalizationRegistry(function(asyncId) {
...
function _restoreContext(ctx) {
	...
	for (var i = 0; i < ctx.length; i++) ctx[i].als._store = ctx[i].store;
	return prev;
}
...
function _wrapPromiseCallback(fn) {
	...
	return function() {
		var prev = _restoreContext(captured);
		try {
			return _fnApply.call(fn, void 0, arguments);
		} finally {
			_restoreContext(prev);
		}
	};
...
(function() {
	...
	Promise.prototype.then = function(onFulfilled, onRejected) {
		return origThen.call(this, typeof onFulfilled === "function" ? _wrapPromiseCallback(onFulfilled) : onFulfilled, typeof onRejected === "function" ? _wrapPromiseCallback(onRejected) : onRejected);
	};
	if (typeof Promise.prototype["catch"] === "function") {
		var origCatch = Promise.prototype["catch"];
		Promise.prototype["catch"] = function(onRejected) {
			return origCatch.call(this, typeof onRejected === "function" ? _wrapPromiseCallback(onRejected) : onRejected);
		};
	}
	if (typeof Promise.prototype["finally"] === "function") {
		var origFinally = Promise.prototype["finally"];
		Promise.prototype["finally"] = function(onFinally) {
			return origFinally.call(this, typeof onFinally === "function" ? _wrapPromiseCallback(onFinally) : onFinally);
		};
	}
})();
(function() {
	...
	if (typeof globalThis.setTimeout === "function") {
		...
		globalThis.setTimeout = function(fn) {
			...
			if (typeof fn === "function") args.unshift(_wrapCallback(fn));
	...
	if (typeof globalThis.queueMicrotask === "function") {
		...
		globalThis.queueMicrotask = function(fn) {
			if (typeof fn === "function") return origQueueMicrotask.call(globalThis, _wrapCallback(fn));
...
function __emitInit(asyncId, type, triggerAsyncId, resource) {
	...
	for (i = 0; i < _activeHooks.length; i++) {
		...
		if (callbacks && typeof callbacks.init === "function") try {
...
function __emitDestroy(asyncId) {
	...
	for (i = 0; i < _activeHooks.length; i++) {
		var hook = _activeHooks[i];
		var callbacks = hook && hook._callbacks;
		if (callbacks && typeof callbacks.destroy === "function") try {
...
function AsyncResource(type, options) {
	...
	this._triggerAsyncId = options && typeof options.triggerAsyncId === "number" ? options.triggerAsyncId : executionAsyncId();
	this._context = _captureContext();
	this._destroyed = false;
	__emitInit(this._asyncId, this.type, this._triggerAsyncId);
	if (_resourceFinalizer) _resourceFinalizer.register(this, this._asyncId);
}
...
AsyncLocalStorage.prototype.enterWith = function(store) {
	this._store = store;
	this._enabled = true;
	if (_allInstances.indexOf(this) === -1) _allInstances.push(this);
};
...
AsyncLocalStorage.prototype.getStore = function() {
	if (!this._enabled) return void 0;
	return this._store;
};
