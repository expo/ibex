...
conversion immediately following its Get. A constants record performs own-key
enumeration and each value Get/conversion in that order, then sorts only the
owned converted snapshot for canonical transport. Its WebIDL `double`
conversion uses `ToNumber`, so BigInt throws synchronously. Explicit layouts
...
throw synchronously. Omitted constants and entry points remain absent; a
separate adversarial test covers `layout: "auto"` and present values. The
