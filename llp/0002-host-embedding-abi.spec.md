...
- `void ex_hermes_destroy(ExactHermesRuntime*)` `[observed]`
  (`include/exact_runtime.h:41`; `hermes_runtime.cc:1455`).
- `int ex_hermes_eval(runtime, data, len, source_url, is_bytecode, out_value)`
  `[observed]` (`include/exact_runtime.h:56`; `hermes_runtime.cc:1464`) —
...
- `void ex_hermes_free_string(char*)` `[observed]`
  (`include/exact_runtime.h:65`; `hermes_runtime.cc:1809`) — frees any string the
  ABI returns; it is a thin wrapper over `free()`.
- `void ex_hermes_set_host_call(runtime, callback)` `[observed]`
  (`include/exact_runtime.h:154`; `hermes_runtime.cc:1754`) — installs the
...
callbacks (`ex_hermes_set_dispatch_callback` and siblings, lines 109-195), a
debugger surface (lines 216-249), and GC/heap introspection (lines 256-264).
...
- The host callback returns a malloc'd C string whose **first byte is a status
  ...
  (`hermes_runtime.cc:1781-1804`). A NULL or empty return becomes JS `null`.
- The C++ side frees the returned string with `free()` and `JSON.parse`s the
  success payload back into a JS value `[observed]` (`hermes_runtime.cc:1785,
  1802-1804`).

On the JS side this bridge is reached through a single global the runtime layer
calls `globalThis.l` (after minification; authored as the host-call function),
wrapped by `packages/ibex-runtime-js/src/core/host-call-bridge.ts` `[observed]`.

## The Rust host surface

The engine's `ex_host_*` C functions are implemented in Rust in
`src/host/abi.rs` and resolve their behavior through a process-global `Host`
singleton.
...
- `host::install_host(host: Host)` stores (or replaces) the singleton in a
  ...
  (`abi.rs:108-118`, test `install_host_replaces_existing_host` at `abi.rs:1857`).
- `ex_host_install()` is the C entry point that installs a default permissive
  ("legacy"/allow-all) host `[observed]` (`abi.rs:590-592`) — the iOS/Swift path
  calls this; the CLI instead calls `install_host` with a configured `Host`
  `[observed]` (doc comment `abi.rs:586-588`).
- `EXACT_HOST_ABI_VERSION` is `1`, returned by `ex_host_version()` `[observed]`
  (`abi.rs:62, 579-581`).

### The `Host` type

`Host` (`src/host/mod.rs:71-76`) holds a `HostConfig`, an `Arc<CapabilityManager>`,
and an `Arc<ModuleLoader>` `[observed]`. Constructors: `Host::new(config)`,
`Host::default_legacy()` (permissive), `Host::strict()` `[observed]`
(`mod.rs:80, 130, 138`). It exposes `check_capability`, `is_allow_all`, and
`resolve_module` `[observed]` (`mod.rs:146-174`). `SecurityMode` is
`Permissive | Capability | Strict` `[observed]` (`mod.rs:29-38`); the C++ bridge
short-circuits capability checks when `ex_host_is_allow_all()` returns 1
`[observed]` (`abi.rs:597-599`).

### What the host backs

The `ex_host_*` functions in `src/host/abi.rs` implement, against std/rusqlite/
getrandom: filesystem ops (`ex_host_fs_*`, `abi.rs:601-1262`), SQLite (open/
prepare/all/get/run/exec, `abi.rs:1264-1698`), env/time/random
(`abi.rs:1700-1737`), console mirroring (`ex_host_console_log`, `abi.rs:1739`),
capability check/grant/log (`abi.rs:661-727`), and module resolution
(`ex_host_module_resolve`, `abi.rs:730-779` — see [LLP 0004](./0004-module-loading-and-builtins.explainer.md)).
Strings returned to C are malloc'd via `CString::into_raw` and freed by
`ex_host_free_string`/`ex_host_free_buffer` `[observed]` (`abi.rs:782, 652`).
...
- Strings out of the `ex_host_*` API are freed with `ex_host_free_string`; raw
  ...
  `[observed]` (`abi.rs:650-659, 781-789`).
- The `__hostCall` callback's returned string is freed by the C++ side, not the
  host `[observed]` (`hermes_runtime.cc:1785`).

## Lifecycle (observed)

1. Host installs itself first: CLI `install_host(Host::new(...))`, iOS
   `ex_host_install()` `[observed]` (`abi.rs:107, 590`; `mod.rs:586-588`).
...
4. The host drives execution via `ex_hermes_eval` and pumps the loop with
   `ex_hermes_poll(now_ms)` (CLI uses tokio; iOS a display/GCD timer)
   `[observed]` (`include/exact_runtime.h:71-91`, lib doc `src/lib.rs:11-12`).
