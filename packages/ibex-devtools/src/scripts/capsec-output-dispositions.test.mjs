...
describe("LLP 0023 output-disposition dataset", () => {
  ...
  test("accounts for all 7,367 covered surfaces and emits 6,413 context-bound output rows", async () => {
    ...
    expect(catalog.counts).toEqual({
      coverageSurfaces: 7_367,
      outputBearingSurfaces: 5_743,
      structuralOnlySurfaces: 1_555,
      unresolvedSurfaces: 69,
      catalogRows: 6_413,
  ...
  test("derives every host ABI account and output row only from its signature contract", async () => {
    ...
    expect(hostEdges).toHaveLength(324);
  ...
  test("blocks verified promotion while any source surface is unresolved", async () => {
    ...
    ).toThrow(/verified output catalog has 59 unresolved surface accounts/);
  }, 30_000);
  ...
  test("migrates the reviewed policy to exact v2 keys with deterministic accounting", async () => {
    ...
    expect(policy.catalogKeyDigest).toBe(
      "sha256-fRkdH7beBrNy08OUoOrQ7ccBxr3CgwsEHfS3R8J_lvk",
    );
