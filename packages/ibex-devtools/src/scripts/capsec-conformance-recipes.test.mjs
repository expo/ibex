...
const REVIEWED_POST_INITIALIZATION_VALUE_TYPES = new Map([
  ...
  ["exact_crypto:KeyObject.asymmetricKeyDetails", "undefined"],
  ["exact_crypto:KeyObject.asymmetricKeyType", "undefined"],
  ["exact_crypto:KeyObject.symmetricKeySize", "undefined"],
  ["exact_crypto:KeyObject.type", "undefined"],
  ["exact_crypto:X509Certificate.fingerprint", "string"],
  ["exact_crypto:X509Certificate.fingerprint256", "string"],
  ["exact_crypto:X509Certificate.infoAccess", "undefined"],
  ...
  ["exact_crypto:X509Certificate.subjectAltName", "undefined"],
  ["exact_crypto:X509Certificate.validFrom", "string"],
  ["exact_crypto:X509Certificate.validTo", "string"],
  ...
  ["node_stream:PassThrough.destroyed", "boolean"],
  ["node_stream:Readable.destroyed", "boolean"],
  ["node_stream:Stream.destroyed", "boolean"],
  ...
  ["ws:WebSocket.CLOSING", "number"],
  ["ws:WebSocket.CONNECTING", "number"],
  ["ws:WebSocket.OPEN", "number"],
]);
...
describe("exact-target CapSec executable recipes", () => {
  ...
  test("accounts for every obligation exactly once and reports honest residuals", () => {
    ...
    expect(recipes.summary.fullyExecutableFixtures).toBe(3_919);
    ...
    expect(recipes.summary.unresolvedFixtures).toBe(16_636);
  ...
  test("accounts for the Windows candidate without borrowing Apple probes", () => {
    ...
    expect(windowsRecipes.summary.fullyExecutableFixtures).toBe(3_555);
    expect(windowsRecipes.summary.internallyVerifiedFixtures).toBe(3_026);
    expect(windowsRecipes.summary.unresolvedFixtures).toBe(16_673);
  ...
  test("authors source-bound reads for non-capability builtin exports", () => {
    ...
    expect(
      publicReads.every((recipe) => {
        ...
        const reviewedRuntimeTypedValue =
          REVIEWED_POST_INITIALIZATION_VALUE_TYPES.get(
            `${descriptor.sourceKey}:${descriptor.exportName}`,
          ) === descriptor.expectedValueType &&
          new Set(["accessor", "data", "unknown"]).has(descriptor.valueShape);
        const readAccessIsExact =
  ...
  test("binds reviewed globals to armed shared-runtime absence", () => {
    ...
    expect(
      rows
        .filter((recipe) =>
          recipe.terminalObservedKey.startsWith(
            "native-op:global:process.",
          ),
        )
        .map((recipe) => recipe.terminalObservedKey),
    ).toEqual([
      "native-op:global:process.domain",
      "native-op:global:process.__exactAsyncIpcListenerPatch",
      "native-op:global:process.__exactLateIpcListenerPatch",
      "native-op:global:process.__exactProcessIpcBootstrapInstalled",
    ...
    expect(
      rows
        .filter((recipe) =>
          /native-op:global:(?:Bun|Exact)\.unsafe\.(?:gcAggressionLevel|segfault)$/u.test(
            recipe.terminalObservedKey,
          ),
        )
        .map((recipe) => recipe.terminalObservedKey),
    ).toEqual([
      "native-op:global:Bun.unsafe.gcAggressionLevel",
      "native-op:global:Bun.unsafe.segfault",
      "native-op:global:Exact.unsafe.gcAggressionLevel",
  ...
  test("authors bounded normal-return calls for exact non-capability families", () => {
    ...
    const zlibProcessChunkCalls = publicCalls.filter(
      (recipe) =>
        recipe.publicSurfaceProbe.invocation.sourceDescriptor.sourceKey ===
          "node_zlib" &&
        recipe.publicSurfaceProbe.invocation.exportName.endsWith(
          "._processChunk",
        ),
    );
    expect(zlibProcessChunkCalls).toHaveLength(9);
    expect(
      zlibProcessChunkCalls.every(
        (recipe) =>
          recipe.publicSurfaceProbe.invocation.setup.kind ===
          ...
          recipe.publicSurfaceProbe.invocation.arguments[0].kind ===
            "buffer" &&
          recipe.publicSurfaceProbe.invocation.arguments[1].kind === "json" &&
    ...
    const zlibFlushCalls = publicCalls.filter(
      (recipe) =>
        recipe.publicSurfaceProbe.invocation.sourceDescriptor.sourceKey ===
          "node_zlib" &&
        recipe.publicSurfaceProbe.invocation.exportName.endsWith(".flush"),
    );
    expect(zlibFlushCalls).toHaveLength(11);
    expect(
      zlibFlushCalls.every(
        (recipe) =>
          recipe.publicSurfaceProbe.invocation.setup.kind ===
            "zlib-flush-owner" &&
          recipe.publicSurfaceProbe.invocation.setup.callbackPosition ===
            "first-argument" &&
          recipe.publicSurfaceProbe.invocation.setup.flushKind ===
            "default-full-flush" &&
    ...
    const zlibTransformCalls = publicCalls.filter(
      (recipe) =>
        recipe.publicSurfaceProbe.invocation.sourceDescriptor.sourceKey ===
          "node_zlib" &&
        recipe.publicSurfaceProbe.invocation.exportName.endsWith("._transform"),
    );
    expect(zlibTransformCalls).toHaveLength(11);
    expect(
      zlibTransformCalls.every(
        (recipe) =>
          recipe.publicSurfaceProbe.invocation.setup.kind ===
            "zlib-transform-owner" &&
          recipe.publicSurfaceProbe.invocation.setup.inputLength ===
            recipe.publicSurfaceProbe.invocation.arguments[0].bytes.length &&
          recipe.publicSurfaceProbe.invocation.setup.cleanupMethod ===
            "destroy" &&
          recipe.publicSurfaceProbe.invocation.arguments[1].value ===
            "buffer" &&
          recipe.publicSurfaceProbe.invocation.arguments[2].kind ===
  ...
  test("reuses only the exact physically bounded builtin output routes", () => {
    ...
    expect(streamCaptured).toHaveLength(41);
    expect(
      streamCaptured.every(
        (recipe) =>
          recipe.publicSurfaceProbe.invocation.capturedOutputInvocation.route
            .operation === "get" &&
          recipe.publicSurfaceProbe.invocation.capturedOutputInvocation.route
            .cleanup.kind === "stream-owned-destroy",
      ),
    ).toBe(true);
    const streamWebCaptured = captured.filter(
      (recipe) =>
        recipe.publicSurfaceProbe.invocation.sourceDescriptor.sourceKey ===
        "node_stream_web",
    );
    expect(
      streamWebCaptured.map(
        (recipe) =>
          `${recipe.publicSurfaceProbe.invocation.sourceDescriptor.exportName}:${recipe.publicSurfaceProbe.invocation.capturedOutputInvocation.route.operation}`,
      ),
    ).toEqual(["fromWeb:call", "toWeb:call"]);
    ...
    const tlsCaptured = captured.filter(
      (recipe) =>
        recipe.publicSurfaceProbe.invocation.sourceDescriptor.sourceKey ===
        "node_tls",
    );
    expect(
      ...
      "convertALPNProtocols:call",
      "createSecureContext:call",
      "getCACertificates:call",
      "SecureContext:construct",
      "translatePeerCertificate:call",
    ]);
    const assertCaptured = captured.filter(
      (recipe) =>
        recipe.publicSurfaceProbe.invocation.sourceDescriptor.sourceKey ===
        "node_assert",
    );
    ...
    ).toEqual(["doesNotReject:call", "rejects:call"]);
    const fsLifecycleCaptured = captured.filter((recipe) =>
      new Set(["node_fs", "node_fs_promises"]).has(
        recipe.publicSurfaceProbe.invocation.sourceDescriptor.sourceKey,
      ),
    );
    expect(
      fsLifecycleCaptured.map(
        (recipe) =>
          `${recipe.publicSurfaceProbe.invocation.sourceDescriptor.sourceKey}:${recipe.publicSurfaceProbe.invocation.sourceDescriptor.exportName}:${recipe.publicSurfaceProbe.invocation.capturedOutputInvocation.route.operation}`,
      ),
    ).toEqual([
      "node_fs:Dir.close:call",
      "node_fs:Dir.closeSync:call",
      "node_fs:FSWatcher:construct",
      "node_fs_promises:FileHandle:construct",
      "node_fs_promises:FileHandle.close:call",
    ]);
