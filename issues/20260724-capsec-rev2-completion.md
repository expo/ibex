...
Checkpoint synchronous Windows stat, then continue the installed Windows
filesystem audit with the next bounded retained-object effect slice. Prefer
`lstat` only with final-link no-follow identity retained through disclosure, or
`readdir` only with repeat authorization before every disclosed entry and the
existing Windows alias coordinate preserved. Keep worker-backed read residual
until it can recheck authority generation between chunks, and do not advertise
Windows while other installed filesystem routes and 17,981 exact-target
public-evidence rows remain unresolved.
