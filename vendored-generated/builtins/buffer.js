...
function getDetachedArrayBuffers() {
	if (typeof globalThis === "undefined" || typeof WeakSet !== "function") return null;
	var detached = globalThis[detachedArrayBuffersSymbol];
	if (!detached || typeof detached.has !== "function") try {
		detached = /* @__PURE__ */ new WeakSet();
...
function getArrayBufferBacking(value) {
	...
	if (objectToString.call(value) === "[object SharedArrayBuffer]" && value._buffer && objectToString.call(value._buffer) === "[object ArrayBuffer]") return value._buffer;
	return value;
}
...
function throwIfDetachedBufferView(value) {
	if (value && typeof value === "object" && isDetachedArrayBuffer(value.buffer)) throw new TypeError("Cannot operate on a detached ArrayBuffer");
}
function formatReceivedValue(value) {
	...
	if (type !== "object") return "type " + type + " (" + String(value) + ")";
...
function getWritableByteView(value) {
	...
	if (typeof ArrayBuffer !== "undefined" && ArrayBuffer.isView && ArrayBuffer.isView(value)) return new Uint8Array(value.buffer, value.byteOffset || 0, value.byteLength);
	return null;
}
...
function getFillBytes(value, encoding) {
	...
	if (value && value.__isExactBuffer) return value.length > 0 ? value : null;
	if (ArrayBuffer.isView(value)) return value.byteLength > 0 ? new Uint8Array(value.buffer, value.byteOffset, value.byteLength) : null;
...
var _Uint8Array_fill = typeof Uint8Array !== "undefined" && Uint8Array.prototype.fill ? Uint8Array.prototype.fill : null;
function fillRangeWithBytes(target, fillBytes, start, end) {
	...
	if (!fillBytes || fillBytes.length === 0) {
		...
		for (i = from; i < to; i++) target[i] = 0;
		return target;
	}
	if (_Uint8Array_fill && target instanceof Uint8Array && fillBytes.length === 1) {
		_Uint8Array_fill.call(target, fillBytes[0] & 255, from, to);
		return target;
	}
	...
	for (i = from; i < to; i++) target[i] = fillBytes[(i - from) % fillBytes.length] & 255;
	return target;
}
function toEncodingCheckBytes(value) {
	if (isUint8Array(value)) return new Uint8Array(value.buffer, value.byteOffset || 0, value.byteLength);
	if (typeof ArrayBuffer !== "undefined" && ArrayBuffer.isView && ArrayBuffer.isView(value)) return new Uint8Array(value.buffer, value.byteOffset || 0, value.byteLength);
	if (isArrayBufferLike(value)) return new Uint8Array(getArrayBufferBacking(value));
	throw makeInvalidArgTypeError("input", "an instance of ArrayBuffer, Buffer, TypedArray, or DataView", value);
}
function isAsciiBytes(value) {
	...
	for (var i = 0; i < bytes.length; i++) if ((bytes[i] & 128) !== 0) return returnBooleanLike(constructorTarget, false);
...
function isUtf8Bytes(value) {
	...
	var bytes = toEncodingCheckBytes(value);
	if (bytes.length === 0) return returnBooleanLike(constructorTarget, true);
	...
	while (i < bytes.length) {
		var byte1 = bytes[i++];
		if ((byte1 & 128) === 0) continue;
		if ((byte1 & 224) === 192) {
			if (i >= bytes.length) return returnBooleanLike(constructorTarget, false);
			if ((bytes[i++] & 192) !== 128 || (byte1 & 254) === 192) return returnBooleanLike(constructorTarget, false);
			continue;
		}
		if ((byte1 & 240) === 224) {
			...
			var byte3a = bytes[i++];
			var byte3b = bytes[i++];
			if ((byte3a & 192) !== 128 || (byte3b & 192) !== 128) return returnBooleanLike(constructorTarget, false);
			var codePoint3 = (byte1 & 15) << 12 | (byte3a & 63) << 6 | byte3b & 63;
			if (codePoint3 < 2048 || codePoint3 >= 55296 && codePoint3 <= 57343) return returnBooleanLike(constructorTarget, false);
			continue;
		}
		if ((byte1 & 248) === 240) {
			...
			var byte4b = bytes[i++];
			var byte4c = bytes[i++];
			if ((byte4a & 192) !== 128 || (byte4b & 192) !== 128 || (byte4c & 192) !== 128) return returnBooleanLike(constructorTarget, false);
			var codePoint4 = (byte1 & 7) << 18 | (byte4a & 63) << 12 | (byte4b & 63) << 6 | byte4c & 63;
			if (codePoint4 < 65536 || codePoint4 > 1114111) return returnBooleanLike(constructorTarget, false);
			continue;
		}
		return returnBooleanLike(constructorTarget, false);
	}
...
function createOutOfRangeError(name, expectation, received) {
	var err = /* @__PURE__ */ new RangeError("The value of \"" + name + "\" is out of range. It must be " + expectation + ". Received " + received);
	err.code = "ERR_OUT_OF_RANGE";
	return err;
}
function makeFirstArgumentError(value) {
	var err = /* @__PURE__ */ new TypeError("The first argument must be of type string or an instance of Buffer, ArrayBuffer, or Array or an Array-like Object. Received " + formatReceivedValue(value));
	err.code = "ERR_INVALID_ARG_TYPE";
	return err;
}
function makeBufferBoundsError(which) {
	var err = /* @__PURE__ */ new RangeError("\"" + which + "\" is outside of buffer bounds");
...
function fromArrayLike(value) {
	...
	for (var i = 0; i < length; i++) bytes[i] = value[i] & 255;
	return bytes;
}
function fromArrayBuffer(value, byteOffset, length) {
	var backing = getArrayBufferBacking(value);
	var totalLength = getArrayBufferByteLength(value);
	if (totalLength === null) throw makeFirstArgumentError(value);
	totalLength = totalLength >>> 0;
	var offset = normalizeInteger(byteOffset, 0);
	if (!isFiniteNumber(offset) || offset < 0 || offset > totalLength) throw makeBufferBoundsError("offset");
	var viewLength;
	if (length === void 0) viewLength = totalLength - offset;
	else {
		viewLength = normalizeInteger(length, 0);
		if (!isFiniteNumber(viewLength) || viewLength < 0 || offset + viewLength > totalLength) throw makeBufferBoundsError("length");
	}
...
function formatBigIntWithSeparators(value) {
	...
	if (prefixLength > 0) formatted = digits.slice(0, prefixLength);
...
function decodeBase64Bytes(value) {
	...
	if (remainder === 2) {
		...
		bytes[outIndex++] = (tailD & 15) << 4 | tailE >> 2;
	}
	return bytes;
}
function validateBigIntWrite(value, signed) {
	if (typeof value !== "bigint") {
		var typeErr = /* @__PURE__ */ new TypeError("The \"value\" argument must be of type bigint. Received " + formatReceivedValue(value));
...
function formatNumberWithSeparators(value) {
	return (value < 0 ? "-" : "") + String(Math.abs(value)).replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1_");
}
function makeInvalidArgTypeError(name, expected, value) {
	var err = /* @__PURE__ */ new TypeError("The \"" + name + "\" argument must be " + expected + ". Received " + formatReceivedValue(value));
	err.code = "ERR_INVALID_ARG_TYPE";
	return err;
}
function makeInvalidArgValueError(name, value) {
	var err = /* @__PURE__ */ new TypeError("The argument '" + name + "' is invalid. Received " + String(value));
	err.code = "ERR_INVALID_ARG_VALUE";
	return err;
}
function makeUnknownEncodingError(encoding) {
	var err = /* @__PURE__ */ new TypeError("Unknown encoding: " + encoding);
	err.code = "ERR_UNKNOWN_ENCODING";
	return err;
}
function makeOutOfBoundsReadError() {
	var err = /* @__PURE__ */ new RangeError("Attempt to access memory outside buffer bounds");
	err.code = "ERR_BUFFER_OUT_OF_BOUNDS";
	return err;
}
...
function validateOffset(offset, byteLength, bufferLength) {
	...
	if (!isFiniteNumber(offset) || offset < 0 || offset > maxOffset) {
		if (bufferLength < byteLength && offset >= 0 && isFiniteNumber(offset)) throw makeOutOfBoundsReadError();
		throw createOutOfRangeError("offset", ">= 0 and <= " + maxOffset, offset);
	}
	if (offset + byteLength > bufferLength) throw makeOutOfBoundsReadError();
	return offset;
}
function validateByteLength(byteLength) {
	if (typeof byteLength !== "number") throw makeInvalidArgTypeError("byteLength", "of type number", byteLength);
	if (byteLength !== byteLength || byteLength !== Math.floor(byteLength)) throw createOutOfRangeError("byteLength", "an integer", byteLength);
	if (!isFiniteNumber(byteLength) || byteLength < 1 || byteLength > 6) throw createOutOfRangeError("byteLength", ">= 1 and <= 6", byteLength);
	return byteLength;
}
...
function normalizeCopyIndex(value, name, fallback) {
	...
	var number = Number(value);
	if (number !== number) return fallback;
	if (!isFiniteNumber(number)) throw createOutOfRangeError(name, ">= 0", value);
	return normalizeToInteger(number);
}
function normalizeCompareIndex(value, name, fallback) {
	if (value === void 0) return fallback;
	if (typeof value !== "number") throw makeInvalidArgTypeError(name, "of type number", value);
	if (value !== value || value !== Math.floor(value)) throw createOutOfRangeError(name, "an integer", value);
	if (!isFiniteNumber(value)) throw createOutOfRangeError(name, ">= 0 && <= 2147483647", value);
	return normalizeToInteger(value);
}
function normalizeFillIndex(value, name, fallback, bufferLength) {
	if (value === void 0) return fallback;
	if (typeof value !== "number") throw makeInvalidArgTypeError(name, "of type number", value);
	if (value !== value) throw createOutOfRangeError(name, "an integer", value);
	if (!isFiniteNumber(value) || value < 0 || value > bufferLength) throw createOutOfRangeError(name, ">= 0 && <= " + bufferLength, value);
	return normalizeToInteger(value);
}
...
function decodeUtf8Bytes(bytes) {
	...
	while (i < len) {
		...
		var cp;
		if (seqLen === 2) cp = (b & 31) << 6 | bytes[i + 1] & 63;
		else if (seqLen === 3) cp = (b & 15) << 12 | (bytes[i + 1] & 63) << 6 | bytes[i + 2] & 63;
		else cp = (b & 7) << 18 | (bytes[i + 1] & 63) << 12 | (bytes[i + 2] & 63) << 6 | bytes[i + 3] & 63;
		if (cp < minCp || cp >= 55296 && cp <= 57343 || cp > 1114111) {
			result += "�";
			i++;
			continue;
		}
		if (cp > 65535) {
			cp -= 65536;
			result += String.fromCharCode(55296 + (cp >> 10), 56320 + (cp & 1023));
...
function encodeString(value, encoding) {
	...
	if (enc === "latin1" || enc === "binary") {
		...
		for (var k = 0; k < str.length; k++) lat[k] = str.charCodeAt(k) & 255;
		return lat;
	}
	if (enc === "ascii") {
		var asc = new Uint8Array(str.length);
		for (var m = 0; m < str.length; m++) asc[m] = str.charCodeAt(m) & 255;
		return asc;
	}
	if (enc === "utf16le" || enc === "ucs2" || enc === "ucs-2" || enc === "utf-16le") {
		...
		for (var p = 0; p < str.length; p++) {
			...
			u16[p * 2] = code & 255;
			u16[p * 2 + 1] = code >> 8 & 255;
		}
		return u16;
	}
	if (typeof TextEncoder !== "undefined") return new TextEncoder().encode(str);
	var fallback = new Uint8Array(str.length);
	for (var n = 0; n < str.length; n++) fallback[n] = str.charCodeAt(n) & 255;
	return fallback;
}
function toByteArray(value, encoding) {
	if (value == null) throw makeFirstArgumentError(value);
	if (isUint8Array(value)) return new Uint8Array(value);
	if (typeof ArrayBuffer !== "undefined" && ArrayBuffer.isView && ArrayBuffer.isView(value)) return fromArrayLike(value);
	if (isArrayBufferLike(value)) return new Uint8Array(value);
	if (typeof value === "string") {
		if (encoding !== void 0 && encoding !== null) {
			var encodingName = String(encoding);
			if (typeof Buffer === "function" && Buffer.isEncoding && !Buffer.isEncoding(encodingName)) throw makeUnknownEncodingError(encoding);
		}
		return encodeString(value, encoding);
	}
	if (value && typeof value === "object" && typeof Symbol === "function" && Symbol.toPrimitive && typeof value[Symbol.toPrimitive] === "function") {
		var primitive = value[Symbol.toPrimitive]("string");
		if (primitive !== value && (typeof primitive !== "object" || primitive === null)) return toByteArray(primitive, encoding);
	}
	if (typeof value === "object" && value !== null && typeof value.valueOf === "function") {
		var unboxed = value.valueOf();
		if (unboxed !== value) return toByteArray(unboxed, encoding);
	}
	if (Array.isArray(value)) return new Uint8Array(value);
	if (typeof value === "object" && value !== null && ("length" in value || "buffer" in value)) return fromArrayLike(value);
	if (value && typeof value === "object" && value.type === "Buffer" && Array.isArray(value.data)) return new Uint8Array(value.data);
	throw makeFirstArgumentError(value);
}
function Buffer(value, encoding, length) {
	if (typeof value === "number") {
		if (arguments.length > 1) throw makeInvalidArgTypeError("string", "of type string", value);
...
function makeBuffer(bytes) {
	...
	if (Buffer._protoReady) Object.setPrototypeOf(buffer, Buffer.prototype);
	else {
		for (var key in BufferProto) buffer[key] = BufferProto[key];
...
Buffer.from = function(value, encoding, length) {
	if (typeof value === "number") throw makeFirstArgumentError(value);
	if (value && value.__isExactBuffer) return makeBuffer(new Uint8Array(value));
	if (isArrayBufferLike(value)) return fromArrayBuffer(value, encoding, length);
	if (isUint8Array(value)) return makeBuffer(new Uint8Array(value));
	return makeBuffer(toByteArray(value, encoding));
};
function normalizeBufferSize(size) {
	if (typeof size !== "number") throw makeInvalidArgTypeError("size", "of type number", size);
	if (size !== size) {
		var sizeErr = /* @__PURE__ */ new RangeError("The value of \"size\" is out of range. It must be a non-negative integer. Received " + (size !== size ? "NaN" : size));
		sizeErr.code = "ERR_OUT_OF_RANGE";
		throw sizeErr;
	}
	if (!isFiniteNumber(size) || size < 0 || size > kMaxLength) {
		var rangeErr = /* @__PURE__ */ new RangeError("The value of \"size\" is out of range. It must be >= 0 && <= " + kMaxLength + ". Received " + size);
		rangeErr.code = "ERR_OUT_OF_RANGE";
		throw rangeErr;
	}
...
function getAllocFillBytes(value, encoding) {
	if (encoding !== void 0 && encoding !== null && typeof encoding !== "string") throw makeInvalidArgTypeError("encoding", "of type string", encoding);
	if (typeof value === "string") {
		var enc = encoding == null ? "utf8" : encoding;
		if (typeof Buffer === "function" && Buffer.isEncoding && !Buffer.isEncoding(enc)) throw makeUnknownEncodingError(enc);
		var stringBytes = encodeString(value, enc);
		if (coerceEncoding(enc) === "hex" && value.length > 0 && (value.length % 2 !== 0 || stringBytes.length * 2 !== value.length)) throw makeInvalidArgValueError("value", value);
		if (value.length > 0 && stringBytes.length === 0) throw makeInvalidArgValueError("value", value);
		return stringBytes;
	}
	if (value && value.__isExactBuffer) {
		if (value.length === 0) throw makeInvalidArgValueError("value", value);
		return value;
	}
	if (ArrayBuffer.isView(value)) {
		if (value.byteLength === 0) throw makeInvalidArgValueError("value", value);
		return new Uint8Array(value.buffer, value.byteOffset || 0, value.byteLength);
	}
	if (isArrayBufferLike(value)) {
		var bytes = new Uint8Array(value);
		if (bytes.length === 0) throw makeInvalidArgValueError("value", value);
		return bytes;
	}
...
Buffer.alloc = function(size, fill, encoding) {
	...
	if (fill === void 0 || fill === null) return makeBuffer(bytes);
	fillRangeWithBytes(bytes, getAllocFillBytes(fill, encoding), 0, bytes.length);
...
Buffer.byteLength = function(string, encoding) {
	if (typeof string !== "string" && !(string && string.__isExactBuffer) && !isArrayBufferLike(string) && !ArrayBuffer.isView(string)) {
		var err = /* @__PURE__ */ new TypeError("The \"string\" argument must be of type string or an instance of Buffer or ArrayBuffer. Received " + formatReceivedValue(string));
		err.code = "ERR_INVALID_ARG_TYPE";
		throw err;
	}
	if (typeof string !== "string") {
		if (ArrayBuffer.isView(string) && typeof string.length === "number") {
			var bytesPerElement = string && string.constructor && string.constructor.BYTES_PER_ELEMENT;
			if (typeof bytesPerElement !== "number") bytesPerElement = string.BYTES_PER_ELEMENT;
			if (typeof bytesPerElement === "number") return string.length * bytesPerElement;
		}
		if (ArrayBuffer.isView(string) && typeof string.byteLength === "number") return string.byteLength;
		if (typeof string.byteLength !== "undefined") return string.byteLength;
...
Buffer.compare = function(a, b) {
	if (!isUint8Array(a)) {
		var buf1Err = /* @__PURE__ */ new TypeError("The \"buf1\" argument must be an instance of Buffer or Uint8Array. Received " + formatReceivedValueWithQuotedString(a));
		buf1Err.code = "ERR_INVALID_ARG_TYPE";
		throw buf1Err;
	}
	if (!isUint8Array(b)) {
		var buf2Err = /* @__PURE__ */ new TypeError("The \"buf2\" argument must be an instance of Buffer or Uint8Array. Received " + formatReceivedValueWithQuotedString(b));
...
Buffer.concat = function(list, totalLength) {
	if (!Array.isArray(list)) {
		var listErr = /* @__PURE__ */ new TypeError("The \"list\" argument must be an instance of Array. Received " + formatReceivedValue(list));
	...
	if (total === void 0) {
		...
		for (var i = 0; i < list.length; i++) {
			if (!isUint8Array(list[i])) {
				var itemErr = /* @__PURE__ */ new TypeError("The \"list[" + i + "]\" argument must be an instance of Buffer or Uint8Array. Received " + formatReceivedValue(list[i]));
		...
		if (typeof total !== "number") throw makeInvalidArgTypeError("length", "of type number", total);
		if (total !== Math.floor(total)) throw createOutOfRangeError("length", "an integer", total);
		if (total < 0 || total > Number.MAX_SAFE_INTEGER) throw createOutOfRangeError("length", ">= 0 && <= 9007199254740991", total);
		for (var j = 0; j < list.length; j++) if (!isUint8Array(list[j])) {
			var listItemErr = /* @__PURE__ */ new TypeError("The \"list[" + j + "]\" argument must be an instance of Buffer or Uint8Array. Received " + formatReceivedValue(list[j]));
...
Buffer.of = function() {
	...
	for (var i = 0; i < arguments.length; i++) bytes[i] = arguments[i] & 255;
	return makeBuffer(bytes);
};
...
function toSearchBytes(value, encoding) {
	...
	if (typeof value === "number") {
		...
		return new Uint8Array([number & 255]);
	}
	if (isUint8Array(value)) return value;
	if (typeof ArrayBuffer !== "undefined" && ArrayBuffer.isView && ArrayBuffer.isView(value)) return new Uint8Array(value.buffer, value.byteOffset || 0, value.byteLength);
	if (isArrayBufferLike(value)) return new Uint8Array(getArrayBufferBacking(value));
	throw makeInvalidArgTypeError("value", "of type string, number, Buffer, Uint8Array, or ArrayBuffer", value);
}
function normalizeIndexOfOffset(bufferLength, byteOffset) {
	if (byteOffset === void 0) return 0;
...
function normalizeLastIndexOfOffset(bufferLength, byteOffset) {
	...
	if (byteOffset === void 0) return bufferLength - 1;
	var offset = Number(byteOffset);
	if (offset !== offset) return bufferLength - 1;
	if (!isFiniteNumber(offset)) return offset < 0 ? -1 : bufferLength - 1;
	offset = normalizeToInteger(offset);
	if (offset < 0) offset += bufferLength;
...
function findBufferSequence(buffer, value, byteOffset, encoding, backwards) {
	...
	if (backwards) {
		...
		for (var i = start; i >= 0; i--) if (compareBytes(buffer, needle, i, i + needle.length, 0, needle.length) === 0) return i;
		return -1;
	}
	var end = buffer.length - needle.length;
	for (var j = offset; j <= end; j++) if (compareBytes(buffer, needle, j, j + needle.length, 0, needle.length) === 0) return j;
	return -1;
}
function getSliceView(target) {
	var view = getWritableByteView(target);
	if (!view) throw makeInvalidArgTypeError("this", "an instance of Buffer or Uint8Array", target);
	return view;
}
function normalizeSliceBound(value, name, fallback, length) {
	...
	if (typeof value !== "number") throw makeInvalidArgTypeError(name, "of type number", value);
	if (value !== value) throw createOutOfRangeError(name, "an integer", value);
	if (!isFiniteNumber(value) || value < 0 || value > length) throw createOutOfRangeError(name, ">= 0 && <= " + length, value);
	return normalizeToInteger(value);
}
function sliceWithEncoding(target, encoding, start, end) {
	...
	if (encoding === "hex" && (actualEnd - actualStart) * 2 > kStringMaxLength) throw new Error("Cannot create a string longer than " + kStringMaxLength + " characters");
	return decodeBytes(view, encoding, actualStart, actualEnd);
}
BufferProto.toString = function(encoding, start, end) {
	var enc;
	if (encoding === void 0) enc = "utf8";
	else {
		enc = String(encoding);
		if (!Buffer.isEncoding(enc)) throw makeUnknownEncodingError(enc);
	}
	var actualStart;
	if (start === void 0) actualStart = 0;
	else {
		var startNumber = Number(start);
		if (startNumber !== startNumber) startNumber = 0;
		if (!isFinite(startNumber)) actualStart = startNumber < 0 ? 0 : this.length;
		else {
	...
	var actualEnd;
	if (end === void 0) actualEnd = this.length;
	else {
		var endNumber = Number(end);
		if (endNumber !== endNumber) endNumber = 0;
		if (!isFinite(endNumber)) actualEnd = endNumber < 0 ? 0 : this.length;
		else {
...
BufferProto.equals = function(other) {
	if (!isUint8Array(other)) {
		var otherErr = /* @__PURE__ */ new TypeError("The \"otherBuffer\" argument must be an instance of Buffer or Uint8Array. Received " + formatReceivedValueWithQuotedString(other));
...
BufferProto.toJSON = function() {
	...
	return {
		type: "Buffer",
		data: Array.prototype.slice.call(new Uint8Array(view.buffer, view.byteOffset || 0, view.byteLength))
	};
};
...
BufferProto.copy = function(target, targetStart, sourceStart, sourceEnd) {
	if (!isUint8Array(this)) {
		var thisErr = /* @__PURE__ */ new TypeError("The \"this\" argument must be an instance of Buffer or Uint8Array. Received " + formatReceivedValue(this));
		thisErr.code = "ERR_INVALID_ARG_TYPE";
		throw thisErr;
	}
	var targetBytes = getWritableByteView(target);
	if (!targetBytes) {
		var targetErr = /* @__PURE__ */ new TypeError("The \"target\" argument must be an instance of Buffer or Uint8Array. Received " + formatReceivedValue(target));
...
function clampEncodedWriteLength(bytes, limit, encoding) {
	...
	var enc = coerceEncoding(encoding || "utf8");
	if (enc === "utf16le" || enc === "ucs2" || enc === "ucs-2" || enc === "utf-16le" || enc === "utf16be") return safeLimit - safeLimit % 2;
	if (enc !== "utf8") return safeLimit;
	var completeLength = 0;
	while (completeLength < safeLimit) {
		var byte = bytes[completeLength];
		var sequenceLength = 1;
		if ((byte & 128) === 0) sequenceLength = 1;
		else if ((byte & 224) === 192) sequenceLength = 2;
		else if ((byte & 240) === 224) sequenceLength = 3;
		else if ((byte & 248) === 240) sequenceLength = 4;
		if (completeLength + sequenceLength > safeLimit) break;
...
function writeEncodedValue(targetValue, value, offset, length, encoding) {
	...
	if (!target) {
		var thisErr = /* @__PURE__ */ new TypeError("The \"this\" argument must be an instance of Buffer or Uint8Array. Received " + formatReceivedValue(targetValue));
		thisErr.code = "ERR_INVALID_ARG_TYPE";
		throw thisErr;
	}
	var offsetNumber = offset === void 0 ? 0 : Number(offset);
	var offsetWasNaN = offset !== void 0 && offsetNumber !== offsetNumber;
	if (offsetWasNaN) offsetNumber = 0;
	if (!isFiniteNumber(offsetNumber)) offsetNumber = offsetNumber < 0 ? 0 : target.length;
	else offsetNumber = normalizeInteger(offsetNumber);
	if (offsetNumber < 0 || offsetNumber > target.length) throw makeBufferBoundsError("offset");
	var remaining = target.length - offsetNumber;
	var lengthNumber;
	if (length === void 0) lengthNumber = remaining;
	else {
		lengthNumber = Number(length);
		if (lengthNumber !== lengthNumber) lengthNumber = remaining;
		else if (!isFiniteNumber(lengthNumber)) lengthNumber = lengthNumber < 0 ? 0 : remaining;
		else {
			lengthNumber = normalizeInteger(lengthNumber);
			if (lengthNumber < 0) lengthNumber = 0;
		}
		if (lengthNumber > remaining) if (offsetWasNaN) lengthNumber = remaining;
		else throw makeBufferBoundsError("length");
	}
	var bytes = encoding === "utf16be" ? encodeUtf16beString(value) : encodeString(value, encoding);
	var bytesToWrite = clampEncodedWriteLength(bytes, lengthNumber, encoding);
	for (var i = 0; i < bytesToWrite; i++) target[offsetNumber + i] = bytes[i];
	return bytesToWrite;
}
...
BufferProto.compare = function(target, targetStart, targetEnd, sourceStart, sourceEnd) {
	if (!isUint8Array(target)) {
		var compareErr = /* @__PURE__ */ new TypeError("The \"target\" argument must be an instance of Buffer or Uint8Array. Received " + formatReceivedValueWithQuotedString(target));
...
BufferProto.readUIntLE = function(offset, byteLength) {
	...
	if (offset === void 0) throw makeInvalidArgTypeError("offset", "of type number", offset);
	offset = validateOffset(offset, byteLength, this.length);
	var value = 0;
	for (var i = 0; i < byteLength; i++) value += this[offset + i] * Math.pow(2, 8 * i);
	return value;
};
BufferProto.readUIntBE = function(offset, byteLength) {
	...
	offset = validateOffset(offset, byteLength, this.length);
	var value = 0;
	for (var i = 0; i < byteLength; i++) value = value * 256 + this[offset + i];
	return value;
};
...
BufferProto.readIntLE = function(offset, byteLength) {
	...
	return value >= Math.pow(2, 8 * byteLength - 1) ? value - Math.pow(2, 8 * byteLength) : value;
};
...
BufferProto.writeUInt16LE = function(value, offset) {
	validateWriteValue(value, 0, 65535, 16, false);
	offset = validateOffset(offset, 2, this.length);
	this[offset] = value & 255;
...
BufferProto.writeUInt32LE = function(value, offset) {
	validateWriteValue(value, 0, 4294967295, 32, false);
	offset = validateOffset(offset, 4, this.length);
	this[offset] = value & 255;
	this[offset + 1] = value >>> 8 & 255;
...
BufferProto.writeInt8 = function(value, offset) {
	validateWriteValue(value, -128, 127, 8, true);
	offset = validateOffset(offset, 1, this.length);
	this[offset] = value < 0 ? value + 256 : value;
	return offset + 1;
};
BufferProto.writeInt16LE = function(value, offset) {
	validateWriteValue(value, -32768, 32767, 16, true);
	return this.writeUInt16LE(value < 0 ? value + 65536 : value, offset);
};
BufferProto.writeInt16BE = function(value, offset) {
	validateWriteValue(value, -32768, 32767, 16, true);
	return this.writeUInt16BE(value < 0 ? value + 65536 : value, offset);
};
BufferProto.writeInt32LE = function(value, offset) {
	validateWriteValue(value, -2147483648, 2147483647, 32, true);
	return this.writeUInt32LE(value < 0 ? value + 4294967296 : value, offset);
};
BufferProto.writeInt32BE = function(value, offset) {
	validateWriteValue(value, -2147483648, 2147483647, 32, true);
	return this.writeUInt32BE(value < 0 ? value + 4294967296 : value, offset);
};
...
BufferProto.fill = function(value, start, end, encoding) {
	...
	if (bufferLength === null) throw makeInvalidArgTypeError("this", "an instance of Buffer or Uint8Array", this);
	...
	} else if (end === void 0) end = bufferLength;
	if (encoding !== void 0 && encoding !== null && typeof encoding !== "string") throw makeInvalidArgTypeError("encoding", "of type string", encoding);
...
Buffer.prototype = Object.create(Uint8Array.prototype, { constructor: {
	value: Buffer,
	writable: true,
	configurable: true
} });
...
var kStringMaxLength = 2147483647;
var INSPECT_MAX_BYTES = 50;
var exported = {
	Buffer,
	atob: typeof atob === "function" ? atob : void 0,
	btoa: typeof btoa === "function" ? btoa : void 0,
	SlowBuffer: Buffer,
...
Object.defineProperty(exported, "Blob", {
	get: function() {
		return typeof globalThis.Blob === "function" ? globalThis.Blob : void 0;
	},
	enumerable: true,
	configurable: true
});
...
Object.defineProperty(exported, "INSPECT_MAX_BYTES", {
	...
	set: function(value) {
		if (typeof value !== "number") throw makeInvalidArgTypeError("value", "of type number", value);
		if (value !== value || value < 0) throw createOutOfRangeError("value", ">= 0", value);
